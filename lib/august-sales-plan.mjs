...
const rows = [
  ...
  },
  {
    ...
    baseAdBudget: 0,
    performanceReserve: 0,
    tactic: "8月只承接自然与活动流量；修复证据用于下一轮预算评审。",
    gate: "8月授权广告预算为$0；链接质量、价格和自然转化通过后，只进入下一轮计划评审，不在本月临时加预算。",
    stopRule: "保持$0广告预算；自然与活动访问仍无转化时继续修复，不为目标硬烧。",
  },
  {
    listing: "DMOM1025",
    parts: ["LFC-3B", "LFC-3W"],
    ...
    baseAdBudget: 0,
    performanceReserve: 0,
    tactic: "自然利润补充；修复Listing后观察自然转化，不在8月新增广告。",
    gate: "8月授权广告预算为$0；链接质量和自然订单通过后，进入下一轮计划评审。",
    stopRule: "保持$0广告预算；自然访问仍无转化时继续修复，不临时重测。",
  },
  {
    listing: "DMOM1026",
    parts: ["VFC-2B", "VFC-2W"],
    ...
    baseAdBudget: 0,
    performanceReserve: 0,
    tactic: "薄毛利自然款；8月不新增广告，不承担店铺冲量。",
    gate: "8月授权广告预算为$0；逐Part成本、实时售价和自然转化通过后，进入下一轮计划评审。",
    stopRule: "保持$0广告预算；禁止用薄毛利补广告亏损。",
  },
  {
    listing: "DMOM1016",
    parts: ["5T-wangge"],
...
export function augustSalesPlanForListing(
  listing,
  planRows = AUGUST_SALES_PLAN_ROWS,
) {
  const row = planRows.find(
    (item) => String(item.listing) === String(listing || ""),
  );
  if (!row) return null;
  return {
    targetMetric: AUGUST_SALES_PLAN.targetMetric,
    targetOrders: row.targetOrders,
    baseAdBudget: row.baseAdBudget,
    canaryBudget: row.performanceReserve,
    plannedAdBudget: row.plannedAdBudget,
    scaleEligible: row.plannedAdBudget > 0,
  };
}
...
function isScaleAction(action) {
  const before = objectPayload(action?.before_payload);
  const proposed = objectPayload(action?.proposed_payload);
  if (action?.action_type === "SET_LISTING_ACTIVE") {
    return proposed.active === true && before.active !== true;
  }
  if (action?.action_type === "SET_LISTING_BID") {
    return Number(proposed.bid || 0) > Number(before.bid || 0);
  }
  return false;
}

export function validateAugustAdActionsAgainstPlan(
  actions = [],
  planRows = AUGUST_SALES_PLAN_ROWS,
) {
  for (const action of actions) {
    if (!isScaleAction(action)) continue;
    const plan = augustSalesPlanForListing(action.listing, planRows);
    if (!plan || !plan.scaleEligible) {
      return {
        ok: false,
        listing: String(action.listing || ""),
        reason: "AUGUST_LISTING_AD_BUDGET_ZERO",
      };
    }
  }
  return { ok: true, listing: null, reason: null };
}
...
export const AUGUST_SALES_MILESTONES = Object.freeze([
  ...
  },
  {
    ...
    note: "仅成熟赢家冲刺，禁止为补目标突破当前授权阶段广告上限。",
  },
]);
...
export function summarizeAugustSalesPlan(planRows = AUGUST_SALES_PLAN_ROWS) {
  ...
  return {
    ...
    hardAdCap: AUGUST_EXECUTION_POLICY.stageTwoAdCap,
    modelHardAdCap: roundMoney(modelHardAdCap),
