...
const HEADER_ALIASES = {
  partNumber: ["part_number", "partnumber", "part", "sku", "sku编码", "sku 编码", "货号", "商品编码", "part number", "supplier_part_number", "supplier part number", "供应商货号"],
  unitCost: ["unit_cost", "unitcost", "cost", "unit cost", "成本", "单位成本", "采购成本", "单价成本", "采购价", "usd_cost", "cost_usd"],
};
...
export function validateCostRows(rows, sellingPriceCents = new Map()) {
  ...
  rows.forEach((row, index) => {
    ...
    const rawCost = String(row.unitCost ?? "").trim();
    if (!part && !rawCost) return;
    if (!part) {
