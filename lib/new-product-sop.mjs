...
function contentReadiness(item, signals) {
  ...
  const hasCatalogIssueEvidence = Boolean(
    item?.insights
    && ["problems", "warnings", "opportunities"].some((group) => (
      Array.isArray(item.insights[group])
    )),
  );
  ...
  return {
    imagesComplete: hasExplicitImageCount
      ...
      : hasCatalogIssueEvidence && !hasMissingImageIssue,
    attributesComplete: hasExplicitAttributeCoverage
      ? attributeCoverage >= 1 && !hasMissingAttributeIssue
      : hasCatalogIssueEvidence && !hasMissingAttributeIssue,
...
export function evaluateNewProductPromotionSop(item = {}) {
  ...
  const hasSalesEvidence = Boolean(
    item?.recent30d
    && Number.isFinite(Number(item.recent30d.units)),
  );
  const units30d = hasSalesEvidence ? Number(item.recent30d.units) : 0;
  ...
  if (units30d > 0) {
    return {
      ...
      evidence: { ...evidence, hasSalesEvidence, units30d, catalogStatus: status, listingCount, catalogProblems },
  ...
  if (!hasSalesEvidence) blockers.push("近 30 天销量证据缺失，无法确认新品状态");
