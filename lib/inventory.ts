...
export async function saveInventoryPushRun(db:D1Database,input:{pushId:string;snapshotId:string;status:string;itemCount:number;batchCount:number;completedBatches:number;failedBatches:number;batches:InventoryPushBatchReceipt[];createdAt?:string}) {
  ...
  for(const batch of input.batches){
    ...
    await db.prepare("INSERT OR REPLACE INTO inventory_push_batches(push_id,batch_index,feed_id,handle,status,state,expected_item_count,item_count,error_count,completed_count,processing_count,errors,submitted_at,completed_at,reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(input.pushId,batch.index,feed?.id||null,feed?.handle||null,String(feed?.status||"UNKNOWN"),batch.state,batch.expectedItemCount,feed?.itemCount??null,Number(feed?.errorCount||0),feed?.completedCount??null,feed?.processingCount??null,JSON.stringify(feed?.errors||[]),feed?.submittedAt||null,feed?.completedAt||null,batch.reason||null).run();
  }
}
