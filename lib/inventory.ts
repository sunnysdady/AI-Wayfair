...
import { isInventoryDryRunAccepted } from "./wayfair-inventory-feed.mjs";
...
type InventoryPushBatchReceipt = {
  ...
  feed?:{id?:string;handle?:string;status?:string;submittedAt?:string;completedAt?:string;itemCount?:number;errorCount?:number;completedCount?:number;processingCount?:number;errors?:{key?:string;message?:string}[]};
  state:string;
  reason:string;
};
...
export async function saveInventoryPushRun(db:D1Database,input:{pushId:string;snapshotId:string;status:string;itemCount:number;batchCount:number;completedBatches:number;failedBatches:number;batches:InventoryPushBatchReceipt[];createdAt?:string}) {
  ...
  for(const batch of input.batches){
    ...
    await db.prepare("INSERT INTO inventory_push_batches(push_id,batch_index,feed_id,handle,status,state,expected_item_count,item_count,error_count,completed_count,processing_count,errors,submitted_at,completed_at,reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(push_id,batch_index) DO UPDATE SET feed_id=excluded.feed_id,handle=excluded.handle,status=excluded.status,state=excluded.state,expected_item_count=excluded.expected_item_count,item_count=excluded.item_count,error_count=excluded.error_count,completed_count=excluded.completed_count,processing_count=excluded.processing_count,errors=excluded.errors,submitted_at=excluded.submitted_at,completed_at=excluded.completed_at,reason=excluded.reason").bind(input.pushId,batch.index,feed?.id||null,feed?.handle||null,String(feed?.status||"UNKNOWN"),batch.state,batch.expectedItemCount,feed?.itemCount??null,Number(feed?.errorCount||0),feed?.completedCount??null,feed?.processingCount??null,JSON.stringify(feed?.errors||[]),feed?.submittedAt||null,feed?.completedAt||null,batch.reason||null).run();
  }
}
...
export async function loadAcceptedInventoryDryRun(db:D1Database,snapshotId:string) {
  ...
  for(const candidate of candidates.results||[]){
    const run=await loadInventoryPushRun(db,candidate.id);
    if(run&&isInventoryDryRunAccepted(run.batches))return candidate;
  }
  return null;
}
