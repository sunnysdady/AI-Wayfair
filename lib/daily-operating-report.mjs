...
export function dailyOperatingReportDue({
  ...
  return lingxingHour(now) >= 20 && existingReportDate !== reportDate;
}

function operationDate(operation) {
  const timestamp = operation?.updatedAt || operation?.closedAt || "";
  const parsed = new Date(timestamp);
  return Number.isNaN(parsed.getTime()) ? "" : lingxingDate(parsed);
}
...
export function buildDailyOperatingReport({
  ...
  const policy = planProgress?.augustArchive?.executionPolicy
    || planProgress?.nextPlan?.executionPolicy
    || {};
  const targetMonth = reportDate.slice(0, 7);
  const orderTarget = targetMonth === "2026-08"
    ? number(policy.stretchOrderTarget) || 150
    : number(planProgress?.plan?.orderTarget)
      || number(planProgress?.septemberPlan?.plan?.orderTarget)
      || 150;
  ...
  const adsLayer = String(dailyAds?.cache?.layer || "UNKNOWN");
  const adsSnapshotLayer = /^POSTGRESQL_/.test(adsLayer);
  const risks = [
    ...(adsRefreshFallback
      ? [`广告实时刷新失败，已使用服务器快照：${adsRefreshFallback}`]
      : adsSnapshotLayer
        ? [`广告指标来自服务器持久化快照：${adsLayer}`]
      : []),
  ...
  return {
    ...
    tomorrow,
    dataQuality: {
      adsFresh: !adsRefreshFallback && !adsSnapshotLayer,
      adsLayer,
