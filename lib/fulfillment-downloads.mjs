...
// A database row can outlive its object-store file after a failed or interrupted
// archival. Keep the healthy labels downloadable and report the exact orders
// that still need re-synchronization.
export function partitionStoredLabelFiles(results = []) {
  const available = [];
  const missingOrderNumbers = [];
  for (const result of results) {
    if (result?.object) available.push(result);
    else if (result?.record?.orderNumber) missingOrderNumbers.push(String(result.record.orderNumber));
  }
  return { available, missingOrderNumbers };
}
