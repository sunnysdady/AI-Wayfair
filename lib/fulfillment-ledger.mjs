...
export const FULFILLMENT_STATUSES = ["待获取面单", "SKU待映射", "已归档面单", "待出库", "已出库", "已发货", "异常"];
...
export async function listFulfillmentRecords(db, filters = {}) {
  ...
  const predicates = [`order_date >= (CAST(? AS DATE)::timestamp AT TIME ZONE '${LINGXING_TIME_ZONE}')`];
  const values = [start];
  if (end) { predicates.push(`order_date < ((CAST(? AS DATE) + INTERVAL '1 day')::timestamp AT TIME ZONE '${LINGXING_TIME_ZONE}')`); values.push(end); }
