...
export const FULFILLMENT_STATUSES = ["待获取面单", "待平台生成面单", "面单待核验", "SKU待映射", "已归档面单", "待出库", "已出库", "已发货", "已取消", "异常"];

const TEXT_FIELDS = {
  sourceKey: ["来源键", 242, true],
  source: ["来源", 80, true],
  orderDate: ["日期时间", 35, false],
...
function orderDateTime(value) {
  ...
  if (Number.isNaN(parsed.getTime())) throw new Error("日期时间必须是有效的 ISO 时间");
...
export function labelArchiveMode(pageCount, recordCount) {
  if (!Number.isInteger(pageCount) || pageCount < 1 || !Number.isInteger(recordCount) || recordCount < 1) return "error";
  if (pageCount === 1 && recordCount === 1) return "whole";
  return pageCount === recordCount ? "split" : "error";
}

export function normalizePhone(value) {
  // Wayfair can provide alternatives such as "6187927946 or 5550100".
  // Keep the primary number and remove non-phone annotation text.
  const primary = String(value || "").split(/\s+\bor\b\s*/i)[0].trim();
  return primary.replace(/[^\d+()\-\.\s]/g, "").trim();
}
...
export function validateFulfillmentRecord(input = {}) {
  ...
  normalized.orderDate = orderDateTime(input.orderDate);
...
export function splitOrderLines(order, items = [], skuMappings = new Map()) {
  ...
  return units.map((unit, index) => {
    ...
    return {
      ...
      orderDate: orderDateTime(order?.poDate),
      ...
      phone: normalizePhone(address?.phoneNumber || address?.phone || order?.customerPhone),
      warehouseSkuCode,
      trackingNumber: "",
      sku: unit.sku,
      quantity: 1,
...
function rowToRecord(row) {
  return {
    ...
    orderDate: orderDateTime(row.orderDate),
...
const FULFILLMENT_RECORD_SELECT = `SELECT
...
/**
 * @param {{start?: string, end?: string, status?: string, limit?: number|string}} filters
 */
export function parseFulfillmentFilters(filters = {}) {
  const { start, end, status, limit = 500 } = filters;
...
export async function listFulfillmentRecords(db, filters = {}) {
  ...
  const predicates = [`order_date >= (?::date::timestamp AT TIME ZONE '${LINGXING_TIME_ZONE}')`];
  const values = [start];
  if (end) { predicates.push(`order_date < ((?::date + INTERVAL '1 day')::timestamp AT TIME ZONE '${LINGXING_TIME_ZONE}')`); values.push(end); }
  if (status) { predicates.push("shipping_status = ?"); values.push(status); }
  values.push(limit);
  const stored = await db.prepare(`${FULFILLMENT_RECORD_SELECT} WHERE ${predicates.join(" AND ")}
    ORDER BY order_date DESC NULLS LAST, order_number ASC LIMIT ?`).bind(...values).all();
  return stored.results.map(rowToRecord);
}

export async function listFulfillmentRecordsForExport(db, filters = {}) {
  const { start, end, status } = parseFulfillmentFilters(filters);
  const predicates = [`order_date >= (?::date::timestamp AT TIME ZONE '${LINGXING_TIME_ZONE}')`];
  const values = [start];
  if (end) { predicates.push(`order_date < ((?::date + INTERVAL '1 day')::timestamp AT TIME ZONE '${LINGXING_TIME_ZONE}')`); values.push(end); }
  if (status) { predicates.push("shipping_status = ?"); values.push(status); }
  const stored = await db.prepare(`${FULFILLMENT_RECORD_SELECT} WHERE ${predicates.join(" AND ")}
    ORDER BY order_date DESC NULLS LAST, order_number ASC LIMIT 10001`).bind(...values).all();
  if (stored.results.length > 10_000) throw new Error("当前日期范围超过 10000 个包裹，请缩小日期范围后下载");
  return stored.results.map(rowToRecord);
}

export async function listFulfillmentRecordsBySourceKeys(db, sourceKeys = []) {
  const keys = [...new Set(sourceKeys.map((value) => String(value || "").trim()).filter((value) => value.startsWith("wayfair:")))];
  if (!keys.length) return [];
  if (keys.length > 2_000) throw new Error("一次最多读取 2000 个履约订单");
  const placeholders = keys.map(() => "?").join(",");
  const stored = await db.prepare(`${FULFILLMENT_RECORD_SELECT} WHERE source_key IN (${placeholders})`).bind(...keys).all();
  return stored.results.map(rowToRecord);
}
