...
export function splitOrderLines(order, items = []) {
...
export async function listFulfillmentRecords(db, { limit = 500 } = {}) {
  ...
  const stored = await db.prepare(`SELECT
    ...
    FROM fulfillment_order_lines ORDER BY order_date DESC NULLS LAST, order_number ASC LIMIT ?`).bind(cappedLimit).all();
  const overrides = new Map(stored.results.map((row) => [String(row.sourceKey), rowToRecord(row)]));
  const source = await db.prepare(`SELECT o.po_number AS poNumber, o.po_date AS poDate,
    i.line_key AS lineKey, i.part_number AS partNumber, i.quantity AS quantity
    FROM orders o JOIN order_items i ON i.po_number = o.po_number
    ORDER BY o.po_date DESC, o.po_number ASC, i.line_key ASC LIMIT ?`).bind(cappedLimit).all();
  const records = [];
  for (const row of source.results) {
    for (const candidate of splitOrderLines(row, [row])) {
      records.push(overrides.get(candidate.sourceKey) || candidate);
      overrides.delete(candidate.sourceKey);
      if (records.length >= cappedLimit) return records;
    }
  }
