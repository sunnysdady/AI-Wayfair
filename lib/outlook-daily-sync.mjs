import { financialDailySummary } from "./email-summary.mjs";
...
function shanghaiDate(value) {
  return new Date(Date.parse(value) + 8 * 60 * 60 * 1000)
    .toISOString()
    .slice(0, 10);
}
...
function sectionsFor(items, tasks) {
  ...
  sections.push({
    title: "检查范围",
    body: "Microsoft Graph 已检查收件箱及名称含 Wayfair 的自定义文件夹，并覆盖上海时间近三日。",
  });
...
export function buildDailyReports(messages, today) {
  ...
  return dates.map((briefDate) => {
    const dayMessages = unique
      .filter((message) => shanghaiDate(message.receivedDateTime) === briefDate)
      .sort((left, right) => String(right.receivedDateTime).localeCompare(String(left.receivedDateTime)));
    const items = dayMessages.map((message) => ({
...
export async function syncOutlookDaily({
  ...
  const cutoff = Date.parse(`${addDays(today, -2)}T00:00:00+08:00`);
