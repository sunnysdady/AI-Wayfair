...
function normalizedLookbackDays(value) {
  const parsed = Number(value);
  if (!Number.isInteger(parsed)) return DEFAULT_LOOKBACK_DAYS;
  return Math.min(MAX_LOOKBACK_DAYS, Math.max(DEFAULT_LOOKBACK_DAYS, parsed));
}
...
function sectionsFor(items, tasks, lookbackDays = DEFAULT_LOOKBACK_DAYS) {
  ...
  sections.push({
    title: "检查范围",
    body: `Microsoft Graph 已检查收件箱及名称含 Wayfair 的自定义文件夹，并覆盖领星站点时间近${lookbackDays + 1}日。`,
  });
...
export function buildDailyReports(messages, today, lookbackDays = DEFAULT_LOOKBACK_DAYS) {
  const effectiveLookbackDays = normalizedLookbackDays(lookbackDays);
  const dates = Array.from(
    { length: effectiveLookbackDays + 1 },
    (_unused, index) => addDays(today, -index),
  );
  ...
  return dates.map((briefDate) => {
    ...
    return {
      ...
      items,
      tasks,
      sections: sectionsFor(items, tasks, effectiveLookbackDays),
    };
  });
}
...
export async function syncOutlookDaily({
  ...
  const effectiveLookbackDays = normalizedLookbackDays(lookbackDays);
  const cutoff = Date.parse(lingxingDayStart(addDays(today, -effectiveLookbackDays)));
  ...
  const reports = buildDailyReports(messages, today, effectiveLookbackDays);
  await db.prepare(
