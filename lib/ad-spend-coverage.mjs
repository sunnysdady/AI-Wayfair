import { lingxingDate } from "./lingxing-business-time.mjs";
...
function reportingDate(timestamp) {
  const parsed = Date.parse(timestamp);
  if (Number.isNaN(parsed)) return null;
  return lingxingDate(new Date(parsed));
}

/**
 * @param {object} input
 * @param {string} input.start Inclusive Lingxing business date (YYYY-MM-DD).
 * @param {string} input.end Inclusive Lingxing business date (YYYY-MM-DD).
 ...
 * @param {string} input.asOf Current Lingxing business date; dates on or after it are still accruing.
 */
export function summarizeAdSpendCoverage({ start, end, days = [], rows = [], asOf }) {
  ...
  for (const date of expectedDays) {
    ...
    const refreshedAfterClose = dayClosed && (reportingDate(ledger.get(date)) || "") > date;
