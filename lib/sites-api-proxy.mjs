...
const AUTH_EXPIRED_MESSAGE = "Sites 数据服务授权已失效，请更新服务端连接凭证。";
...
export async function proxySitesApi(request, env = process.env, fetchImpl = fetch) {
  ...
  try {
    ...
    if ((response.status === 401 || response.status === 403) && !contentType.toLowerCase().includes("application/json")) {
      return jsonError(AUTH_EXPIRED_MESSAGE, 502);
    }
