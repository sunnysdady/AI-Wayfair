...
export function classifyInventoryFeed(feed, expectedItemCount) {
  ...
  if (itemCount !== expectedItemCount) {
    return { state: "failed", reason: `Wayfair 确认 ${itemCount ?? "未知"} 条，预期 ${expectedItemCount} 条` };
  }
  if (FAILED_STATUSES.has(status)) return { state: "failed", reason: `Wayfair feed 状态为 ${status}` };
  if (COMPLETED_STATUSES.has(status) && feed.completedAt) return { state: "completed", reason: "" };
