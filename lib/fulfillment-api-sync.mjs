...
const LABEL_QUERY = `query FulfillmentLabels($numbers: [String!]) {
  labelGenerationEvents(filters: [{ field: poNumber, in: $numbers }], limit: 1000, offset: 0) {
    poNumber
    consolidatedShippingLabel { url }
    shippingLabelInfo { poNumber fullPoNumber numberOfLabels carrier carrierCode trackingNumber }
  }
}`;
const HISTORICAL_LABEL_QUERY = `query HistoricalLabelCheck($before: IsoDateTime!) {
...
function keepMoreAdvancedStatus(current, next) {
  return ["已发货", "已出库"].includes(current) ? current : next;
}
...
async function enrichLabels({ env, token, records }) {
  ...
  const numbers = parents.map(labelNumber).filter(Boolean);
  if (!numbers.length) return { archived: 0, checked: 0 };
  const data = await graphQL(token, LABEL_QUERY, { numbers });
  const events = data.labelGenerationEvents || [];
  const byNumber = new Map(events.map((event) => [String(event.poNumber || ""), event]));
  let archived = 0;
  for (const parent of parents) {
    const recordsForParent = groups.get(parent);
    if (recordsForParent.every((record) => record.labelObjectKey)) continue;
    const event = byNumber.get(labelNumber(parent));
