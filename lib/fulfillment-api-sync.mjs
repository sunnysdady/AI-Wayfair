...
function labelNumber(poNumber) {
  // The label-event filter accepts only the nine-digit numeric portion of a PO.
...
// Registration is different from label lookup: Wayfair requires the complete
// two-letter plus nine-digit PO, e.g. CS677550965.
export function labelRegistrationNumber(poNumber) {
  const match = String(poNumber || "").trim().match(/^[A-Za-z]{2}\d{9}$/);
...
export function trackingAssignments(trackingNumbers, recordCount) {
  if (!Number.isInteger(recordCount) || recordCount < 1) return [];
  const numbers = [...new Set((trackingNumbers || []).map((value) => String(value || "").trim()).filter(Boolean))];
  if (numbers.length === 1) return Array(recordCount).fill(numbers[0]);
  return numbers.length === recordCount ? numbers : [];
}

// Wayfair can return several 4×6 carrier-label pages for a single parcel
// (for example, a Canada shipment with duplicate carrier copies).  These are
// safe to retain together: they all belong to the one PO and are not an
// un-attributable consolidated document.
export function canArchiveMultiPageSingleParcel(pageSizes, recordCount) {
  if (recordCount !== 1 || !Array.isArray(pageSizes) || pageSizes.length < 2) return false;
  return pageSizes.every(({ width, height }) => Math.abs(Number(width) - 288) < 1 && Math.abs(Number(height) - 432) < 1);
}
...
async function splitAndArchivePdf({ files, token, url, records }) {
  ...
  const wholeDocument = mode === "whole"
    || canArchiveMultiPageSingleParcel(source.getPages().map((page) => page.getSize()), records.length);
  if (mode === "error" && !wholeDocument) return { status: "面单待核验", archived: 0 };
  if (wholeDocument) {
...
function labelFailureReason(error) {
  const message = error instanceof Error ? error.message : "Wayfair 面单处理失败";
  return message.replace(/[\u0000-\u001f\u007f]+/g, " ").trim().slice(0, 240) || "Wayfair 面单处理失败";
}

export function isCancelledOrderError(error) {
  const message = error instanceof Error ? error.message : String(error || "");
  return /\border\s+has\s+been\s+cancelled\b|\bcancel(?:led|lation)?\b|订单已取消/i.test(message);
}

async function registerLabelEvent(token, parentOrderNumber) {
  const poNumber = labelRegistrationNumber(parentOrderNumber);
...
async function archiveLabelEvent({ env, token, event, parentOrderNumber, records, trackingOwners }) {
  ...
  const assignments = trackingAssignments(trackingNumbers, records.length);
  for (const [index, trackingNumber] of assignments.entries()) {
    records[index].trackingNumber = trackingNumber;
    trackingOwners.set(trackingNumber, new Set([parentOrderNumber]));
  }
...
async function enrichLabels({ env, token, records }) {
  ...
  if (!parents.some((parent) => labelLookupNumbers(parent).length)) {
    return { archived: 0, checked: 0, matched: 0, ready: 0, unavailable: 0, requested: 0, errors: 0, cancelled: 0, attempts: 0, failureReasons: [] };
  }
  let archived = 0;
  let matched = 0;
  let ready = 0;
  ...
  for (const parent of parents) {
    ...
    if (!event) {
      ...
      try {
        ...
        if (isCancelledOrderError(error)) {
          cancelled += 1;
          for (const record of recordsForParent) record.shippingStatus = "已取消";
          continue;
        }
        errors += 1;
        if (failureReasons.length < 8) failureReasons.push({ parentOrderNumber: parent, reason: labelFailureReason(error) });
    ...
    matched += 1;
    const assignments = trackingAssignments(eventLabelInfo(event).map((item) => item?.trackingNumber), recordsForParent.length);
    for (const [index, trackingNumber] of assignments.entries()) {
      if (!recordsForParent[index].trackingNumber) recordsForParent[index].trackingNumber = trackingNumber;
      trackingOwners.set(trackingNumber, new Set([parent]));
    }
  ...
  return { archived, checked: parents.length, matched, ready, unavailable, requested, errors, cancelled, attempts, failureReasons };
}
