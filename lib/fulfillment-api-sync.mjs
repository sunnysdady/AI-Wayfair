...
async function enrichLabels({ env, token, records }) {
  ...
  if (!parents.some((parent) => labelLookupNumbers(parent).length)) {
    return { archived: 0, checked: 0, matched: 0, ready: 0, unavailable: 0, errors: 0, attempts: 0 };
  }
  let archived = 0;
  let matched = 0;
  let ready = 0;
  ...
  for (const parent of parents) {
    ...
    for (const number of labelLookupNumbers(parent)) {
      ...
      const candidate = selectLabelEvent(data.labelGenerationEvents || []);
    ...
    // The API can return a consolidated document under more than one PO. We use the
    // server-side filter to find candidates, then verify the PDF page count before archiving.
    if (!event) {
      unavailable += 1;
      for (const record of recordsForParent) record.shippingStatus = keepMoreAdvancedStatus(record.shippingStatus, "待平台生成面单");
      continue;
    }
    matched += 1;
    ...
    ready += 1;
    try {
      const archiveResult = await splitAndArchivePdf({ files: env.FILES, token, url, records: recordsForParent });
      archived += archiveResult.archived;
      if (!archiveResult.archived) {
        for (const record of recordsForParent) record.shippingStatus = keepMoreAdvancedStatus(record.shippingStatus, archiveResult.status);
      } else if (recordsForParent.length === 1) {
        const trackingNumbers = [...new Set((event.shippingLabelInfo || []).map((item) => String(item?.trackingNumber || "").trim()).filter(Boolean))];
        if (trackingNumbers.length === 1) recordsForParent[0].trackingNumber = trackingNumbers[0];
      }
    }
    catch {
      errors += 1;
  ...
  return { archived, checked: parents.length, matched, ready, unavailable, errors, attempts };
}
...
// could save the same consolidated PDF under each PO, so re-read only these
// suspicious files and clear the mapping when the PDF is not one label page.
// The object itself remains in storage for audit; it simply stops being offered
// as that order's downloadable label.
async function invalidateSharedConsolidatedLabels({ files, records }) {
  ...
  for (const recordsWithTracking of byTrackingNumber.values()) {
    ...
    for (const record of recordsWithTracking) {
      try {
        const stored = await files.get(record.labelObjectKey);
        if (!stored) continue;
        ...
        if (labelArchiveMode(pageCount, 1) === "whole") continue;
        ...
        // Leave the stored mapping untouched unless its PDF page count proves it is unsafe.
      }
    }
  }
  return invalidated;
