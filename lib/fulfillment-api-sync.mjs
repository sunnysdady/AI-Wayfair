...
async function enrichLabels({ env, token, records }) {
  ...
  if (!parents.some((parent) => labelLookupNumbers(parent).length)) {
    return { archived: 0, checked: 0, matched: 0, ready: 0, unavailable: 0, requested: 0, errors: 0, attempts: 0, failureReasons: [] };
  }
  let archived = 0;
  let matched = 0;
  let ready = 0;
  ...
  return { archived, checked: parents.length, matched, ready, unavailable, requested, errors, attempts, failureReasons };
}
