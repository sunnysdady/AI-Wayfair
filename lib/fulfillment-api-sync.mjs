...
export function labelLookupNumbers(poNumber) {
  const fullNumber = String(poNumber || "").trim();
  const numericNumber = labelNumber(fullNumber);
  return numericNumber ? [...new Set([numericNumber, fullNumber])] : [];
}
...
async function splitAndArchivePdf({ files, token, url, records }) {
  ...
  if (mode === "error") return { status: "异常", archived: 0 };
  if (mode === "whole") {
