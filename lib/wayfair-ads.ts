...
export async function cachedAdSpend(db: D1Database | undefined, start: string, end: string) {
  ...
  try {
    ...
    if (rows.length) {
      ...
      return { spend: Number(spend.toFixed(2)), coverage: Number(coverage?.days || 0) >= expected ? "FULL" : "PARTIAL", coveredDays: Number(coverage?.days || 0), expectedDays: expected, source: "POSTGRESQL_DATABASE" };
    }
