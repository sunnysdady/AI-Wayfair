...
export function buildOperatingReadiness(env = {}) {
  ...
  const catalogSupplier = /^\d+$/.test(String(env.WAYFAIR_CATALOG_SUPPLIER_ID || ""))
    ...
    : null;
  const environmentVerified = deployment === "production" && platform === "cloudflare";
  ...
  const lingxingReady = configured(env, ["LINGXING_APP_ID", "LINGXING_APP_SECRET"]);
  const blockers = [];
  if (!environmentVerified) blockers.push("必须明确运行在 Cloudflare production 生产环境");
  ...
  return {
    ...
    sources: [
      ...
      { id: "lingxing", name: "领星库存 API", status: lingxingReady ? "ready" : "blocked", detail: lingxingReady ? "开放接口凭证已配置" : "未配置 AppId / AppSecret", scope: "仓库库存自动拉取" },
      { id: "outlook", name: "Outlook 邮件日报", status: env.OUTLOOK_INGEST_TOKEN ? "ready" : "blocked", detail: env.OUTLOOK_INGEST_TOKEN ? "写入令牌已配置" : "写入令牌未配置", scope: "风险与待办" },
      { id: "persistence", name: "运营数据库", status: platform === "cloudflare" ? "ready" : "blocked", detail: platform === "cloudflare" ? "D1 + R2 运行环境" : "当前平台不提供完整 D1/R2", scope: "订单、广告、库存、执行单与报告" },
    ],
    live: {
