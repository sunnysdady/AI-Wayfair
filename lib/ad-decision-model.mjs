...
function readinessBlockers(unit, asOf) {
  ...
  if (unit.executionPlan && unit.executionPlan.scaleEligible !== true) {
    blockers.push("AUGUST_PLAN_AD_BUDGET_ZERO");
  }
...
function decisionFor(unit, prior, asOf) {
  ...
  const marginRate = marginKnown ? clamp(finite(unit.economics.marginRate), -10, 1) : null;
  ...
  return {
    ...
    executionPlan: unit.executionPlan ? { ...unit.executionPlan } : null,
    operatingState: unit.operatingState
      ? { ...unit.operatingState }
      : null,
    campaignControl: unit.campaignControl
      ? {
          ...unit.campaignControl,
          isolatedProducts: (unit.campaignControl.isolatedProducts || []).map(
            (item) => ({ ...item }),
          ),
        }
      : null,
