...
export function executionGateForAction({ actionType, strategyBlockers = [], hasMonthlyPlan = true, goalEvidenceReliable = true }) {
  if (actionType !== "INCREASE_DAILY_CAP") return [];
  return [
    ...strategyBlockers,
    !hasMonthlyPlan ? "未设置月度预算归属" : "",
    !goalEvidenceReliable ? "缺月度目标进度证据" : "",
  ].filter(Boolean);
}
...
export function recommendCpcAction(input) {
  ...
  const eventWindow = ["prepare", "ramp", "event", "flash"].includes(input.eventPhase);
  ...
  const accelerationCandidate = eventWindow
    && input.julyPaceGap < 0
    && ["scale", "protect"].includes(input.adRole)
    && matureWinner;

  if (accelerationCandidate) {
    const requiredStock = Math.max(0, input.julyRemainingUnits) + Math.max(0, input.augustReserveUnits);
    const blockers = [
      !input.qualityPass ? "链接评分或评论未通过放量线" : "",
      !input.marginKnown ? "缺SKU精确毛利" : "",
      !input.inventoryKnown ? "缺库存快照" : "",
      input.inventoryKnown && input.inventoryCoverDays < 14 ? `库存仅覆盖${input.inventoryCoverDays}天` : "",
      input.inventoryKnown && input.inventoryQuantity < requiredStock
        ? `库存${input.inventoryQuantity}件不足：完成7月后还需保留8月${input.augustReserveUnits}件预留库存`
        : "",
    ].filter(Boolean);
    if (blockers.length) {
      return {
        ...
        label: "保持 Bid，暂不扩量",
        blockers,
        reasons: [...result.reasons, `7月节奏差${input.julyPaceGap}单，但8月目标库存优先受保护`],
      };
    }
    return {
      ...result,
      ...
      reasons: [
        ...result.reasons,
        `7月落后计划${Math.abs(input.julyPaceGap)}单，成熟归因已证明可放量`,
        `库存覆盖7月剩余${input.julyRemainingUnits}件并保留8月${input.augustReserveUnits}件`,
