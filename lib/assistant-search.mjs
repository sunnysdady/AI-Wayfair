...
const SKU_MONTH_ORDERS_SQL = `
  ...
  WHERE items.part_number = requested_sku.value
    AND orders.po_date >= (requested_month.first_day::timestamp AT TIME ZONE 'Asia/Shanghai')
    AND orders.po_date < ((requested_month.first_day + INTERVAL '1 month')::timestamp AT TIME ZONE 'Asia/Shanghai')
`;
