...
const DAILY_SALES_SQL = `
  ...
  WHERE po_date >= (requested_day.value::timestamp AT TIME ZONE 'Asia/Shanghai')
    AND po_date < ((requested_day.value + INTERVAL '1 day')::timestamp AT TIME ZONE 'Asia/Shanghai')
    AND revenue_cents > 0
`;
...
export async function searchAssistantKnowledge(db, rawInput, options = {}) {
  ...
  const dailySales = command ? dailySalesKnowledge(command, search) : null;
  const records = dailySales?.records || (Array.isArray(search.results) ? search.results : []);
  ...
  return {
    answer: dailySales?.answer || answerFor(query, records, sources),
    command,
