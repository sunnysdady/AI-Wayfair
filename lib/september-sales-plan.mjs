...
export const SEPTEMBER_SALES_PLAN_ROWS = Object.freeze(rows.map((row) => {
...
export function summarizeSeptemberSalesPlan(planRows = SEPTEMBER_SALES_PLAN_ROWS) {
  const expectedRevenue = roundMoney(planRows.reduce((total, row) => total + row.expectedRevenue, 0));
  const expectedGrossProfit = roundMoney(planRows.reduce((total, row) => total + row.expectedGrossProfit, 0));
  const projectedPostAdProfit = roundMoney(expectedGrossProfit - SEPTEMBER_SALES_PLAN.adBudget);
  return Object.freeze({
    listingCount: planRows.length,
    targetOrders: planRows.reduce((total, row) => total + row.targetOrders, 0),
