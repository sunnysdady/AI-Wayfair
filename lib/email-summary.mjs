...
export function normalizeEmailBriefItem(item = {}) {
  ...
  const summaryLines = selected.slice(0, 2).map((candidate) => candidate.line);
  const previewLines = [...selected].sort((left, right) => left.index - right.index).map((candidate) => candidate.line);
