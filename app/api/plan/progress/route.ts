...
export async function GET() {
  try {
    ...
    const actual = await env.DB.prepare(`SELECT COUNT(*) AS orders, COALESCE(SUM(units),0) AS units, COALESCE(SUM(revenue_cents),0)/100.0 AS revenue FROM orders WHERE datetime(po_date)>=datetime(?) AND datetime(po_date)<datetime(?)`).bind(from, until).first<{ orders: number; units: number; revenue: number }>();
    const orderItems = await env.DB.prepare(`SELECT i.po_number AS poNumber, i.part_number AS partNumber, i.quantity AS quantity, i.unit_price_cents AS unitPriceCents FROM order_items i JOIN orders o ON o.po_number=i.po_number WHERE datetime(o.po_date)>=datetime(?) AND datetime(o.po_date)<datetime(?)`).bind(from, until).all();
    ...
    const profit = await env.DB.prepare(`SELECT
      ...
      WHERE datetime(o.po_date)>=datetime(?) AND datetime(o.po_date)<datetime(?)`).bind(from, until).first<{ knownProfitCents: number; unknownRevenueCents: number; revenueCents: number }>();
