...
async function salesBySku(partNumbers: string[]) {
  ...
  const result = await env.DB.prepare(`SELECT i.part_number AS partNumber, SUM(i.quantity) AS units,
    ...
    WHERE o.po_date >= ? AND i.part_number IN (${placeholders})
...
export async function GET(request: Request) {
  try {
    ...
    if(env.DB&&!refresh){
      ...
      if(cached&&Date.now()-Date.parse(cached.updated_at)<CATALOG_CACHE_MS){result=JSON.parse(cached.value);cacheLayer="POSTGRESQL_DATABASE";}
    }
    if(!result){
