  ...
  try {
    ...
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO ad_manual_completions(
        task_key,parent_sku,task_id,campaign_id,ad_group,title,status,completed_at,updated_at
      ) VALUES(?,?,?,?,?,?,?,?,?)
      ON CONFLICT(task_key) DO UPDATE SET
        ...
        .bind(record.taskKey, record.parentSku, record.taskId, record.campaignId, record.adGroup, record.title, record.status, completedAt, now),
      env.DB.prepare("INSERT INTO ad_manual_completion_events(id,task_key,event_type,payload,created_at) VALUES(?,?,?,?,?)")
        .bind(crypto.randomUUID(), record.taskKey, record.status === "COMPLETED" ? "COMPLETED" : "REOPENED", JSON.stringify(record), now),
    ]);
