...
function Sources() {
  ...
  return <><Hero eyebrow="DATA SOURCES · PERMISSION CONTROL" title="数据源" text="连接状态来自当前运行环境；未验证时不会显示为生产可用" />
    <section className="card readiness-banner"><div><span>运行环境</span><b>{data?`${data.environment.platform} / ${data.environment.name}`:'检查中'}</b><small>{data?.environment.verified?'生产环境已验证':'未通过生产环境验证'}</small></div><div><span>Supplier 身份</span><b>{data?.identity.verified?'已核对':'未核对'}</b><small>{data?.identity.expectedSupplierIds.length?data.identity.expectedSupplierIds.join(' / '):'未配置允许清单'}</small></div><div><span>正式写入</span><b>{data?.live.ads.allowed&&data?.live.inventory.allowed?'已解锁':'默认锁定'}</b><small>广告与库存分别受独立开关保护</small></div><div><span>Scope Health</span><b>{data?`${data.scopeHealth.summary.healthy} 正常 · ${data.scopeHealth.summary.failed} 失败`:'检查中'}</b><small>依据最近真实 API 调用，不展示密钥</small></div></section>
