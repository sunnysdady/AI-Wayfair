...
type AiCampaignDiagnosis = {
  campaignId: string; name: string; strategy: string; status: string; startDate: string; dailyCap: string; targetRoas: string;
  stage: "LEARNING" | "LEARNING_OVERDUE" | "LEARNED" | "INACTIVE"; priority: "P0" | "P1" | "P2";
  action: "CONTACT_ACCOUNT_MANAGER" | "WAIT_FOR_LEARNING" | "REVIEW_TARGETS" | "KEEP_INACTIVE";
  daysActive: number; orders14d: number; remainingOrders: number; noDailyCap: boolean; significantChangesBlocked: boolean;
  statusConflict: boolean; reportedStatus: string; platformStage: string; platformObservedAt: string; platformSource: string;
  summary: string; guardrail: string; current14d: AdMetric;
};
type AiAdEligibility = {
  listing: string; campaignId: string; parts: string[]; status: "ELIGIBLE" | "NOT_READY" | "BLOCKED"; canLaunch: boolean;
  orders14d: number; orderGap: number; spend14d: number; wsc14d: number; wscRoas14d: number; blockers: string[];
  preLaunchDailyCap: number; targetRoasFloor: number; whenToLaunch: string; guardrail: string;
  inventory: { known: boolean; coverDays: number | null; quantityOnHand: number }; linkQuality: { pass: boolean };
};
...
type AdAnalysis = {
  current: AdMetric; previous: AdMetric; history: ({ date: string } & AdMetric)[]; campaigns: AdCampaign[]; listings: AdListing[]; aiCampaignDiagnostics: AiCampaignDiagnosis[]; aiAdEligibility: AiAdEligibility[]; zombieFindings: ZombieCampaignFinding[];
...
const MANUAL_AD_TASKS = [
  { id: "ai-learning-escalation", priority: "P0", group: "AI学习期", title: "升级处理超期学习 Campaign 660198", detail: "保存 Partner Home 的 Active Learning 提示、Campaign开始日期、最近14天归因订单、花费、tROAS与Daily Cap截图，并联系 Account Manager 核查长期未完成学习的原因。", sku: "DMOM1002 · 8T-kayak", adType: "Product Targeting · AI-TROAS", keywords: "AI Campaign 不执行关键词或否词动作", match: "不适用", bid: "不改单品 Bid", budget: "当前 NO DAILY CAP（风险项）", rule: "学习期内禁止修改 tROAS、Daily Cap 与 Listing；超过14天仍未达到50个归因订单时联系 Account Manager。紧急止损只能由运营审批后暂停整个 Campaign。" },
  { id: "ai-launch-gate", priority: "P0", group: "AI准入", title: "本期不建议新开 AI 广告，先积累可学习订单量", detail: "逐 SKU 检查滚动14天归因订单、库存覆盖、Listing质量与保本ROAS。没有达到50个归因订单的SKU继续用Manual Campaign积累，不以无上限花费换学习量。", sku: "全部 Manual Campaign SKU", adType: "AI-TROAS 新开评估", keywords: "沿用现有 Manual 关键词结构", match: "不适用", bid: "开前不改单品 Bid", budget: "达到准入后，启用前一次性设定 Daily Cap", rule: "只有滚动14天≥50个归因订单、库存≥30天、Listing质量通过时才可进入下一完整14天周期；启用前设置Daily Cap和tROAS，学习期内冻结参数。" },
  { id: "keyword-structure", priority: "P0", group: "关键词布局", title: "建立 Filing Cabinets 收割 Campaign", detail: "在现有 Keyword Campaign 597350 下按 SKU 拆 Ad Group；Exact 只承接已验证词，Phrase 只做低价拓词，避免多个 SKU 抢同一个泛词。该结构只用于 filing cabinets 类目。", sku: "DMOM1021 · LFC-2B-680", adType: "Sponsored Products · Keyword", keywords: "Exact: lateral filing cabinet; 2 drawer filing cabinet", match: "Exact", bid: "$0.45", budget: "$18/日", rule: "连续 7 天 WSC ROAS ≥4× 且 ≥2 单，日预算才加 20%；否则不加价。" },
  { id: "keyword-remap", priority: "P0", group: "关键词布局", title: "将已验证词从退出 SKU 重新映射", detail: "DRCI1007 已合并，不得继续花费。按商品属性将词迁入新 SKU；迁入前逐个核对落地页的抽屉数、尺寸、颜色和库存。", sku: "DMOM1021 / DMOM1022 / DMOM1019", adType: "Sponsored Products · Keyword", keywords: "DMOM1021: lateral filing cabinet; DMOM1022: mobile file cabinet; DMOM1019: 3 drawer file cabinet", match: "Exact", bid: "$0.24 / $0.20 / $0.45", budget: "$12 / $8 / $10 日", rule: "旧 SKU 的所有 Keyword / Product Campaign 保持 $0；只在新品链接与词意完全匹配时开启。" },
  { id: "keyword-winners", priority: "P1", group: "关键词放量", title: "用 Phrase 扩充赢家词的近义需求", detail: "Exact 保留为收割层；Phrase 仅探索相邻词，Search Term 报告每周二复核一次，转化词再下沉到 Exact。", sku: "DMOM1021 · LFC-2B-680；DMOM1019 · VFC-3", adType: "Sponsored Products · Keyword", keywords: "DMOM1021: filing cabinet, file cabinet with lock; DMOM1019: vertical file cabinet, metal file cabinet", match: "Phrase", bid: "$0.20 / $0.30", budget: "$10 / $8 日", rule: "每个 Phrase 累计 30 点击后：WSC ROAS ≥3.5× 转 Exact；20 点击 0 单先暂停该词。" },
  { id: "keyword-stoploss", priority: "P0", group: "否词 / 止损", title: "执行词级止损，先判定商品相关性", detail: "不要用整 Campaign 止损。先检查 Search Term 是否与 SKU 的品类、尺寸和用途匹配；匹配但不转化则检查价格、库存和 Listing。", sku: "DMOM1021 / DMOM1022 / DMOM1019", adType: "Keyword + Search Term Report", keywords: "不相关例：wood file cabinet（非木质 SKU）；printer stand file cabinet（无对应功能）", match: "Negative Exact 优先", bid: "不适用", budget: "$0（否词）", rule: "20 点击 0 单且无商品对应时加 Negative Exact；永不否定自有品牌词或仍有订单的词。" },
  { id: "budget-migration", priority: "P1", group: "预算迁移", title: "从低效 Product 移出预算，转给已过 Gate 的 Keyword", detail: "当前 Product Campaign 以探索为主；预算迁移不增加账户总额，先关闭明显低效 SKU 的新增预算。", sku: "DMOM1000 / DMOM1018 → DMOM1021 / DMOM1019", adType: "Product Targeting → Keyword Targeting", keywords: "仅补给上述 Exact 赢家词", match: "Exact", bid: "维持现有 bid，不追价", budget: "最多转移 $150/月", rule: "Product 连续 7 天 WSC ROAS <3× 或 CPO >$40 才迁移；Keyword 需先满足 ≥3.5× ROAS。" },
  { id: "manual-controls", priority: "P1", group: "Campaign 参数", title: "逐项设定 Cap、tROAS 与 B2B 护栏", detail: "关键词、否词、Campaign Cap 和 tROAS 保留人工执行。每次只改变一个变量，并在执行批次记录修改前数值、修改后数值、负责人和复盘日；禁止为追量同时提 Bid 与加预算。", sku: "DMOM1021 / DMOM1022 / DMOM1019", adType: "Keyword / Product / B2B", keywords: "B2B 只复用已转化 Exact 词", match: "Exact", bid: "单次调整 ≤10%", budget: "Keyword $750/月；B2B $150/月", rule: "B2B 仅库存已确认时启用；tROAS 只用于现有 AI Bidding Campaign，目标不得低于 3.5×。" },
] as const;
...
function actionPayloadText(action: QueuedAdAction) {
  try {
    ...
    if (action.action_type === "PAUSE_CAMPAIGN") return "人工暂停 Campaign";
    if (action.action_type === "CHECK_LISTING_ELIGIBILITY") return "核查 Listing eligibility / 库存";
    if (action.action_type === "CHECK_LOW_DELIVERY") return "选择 7 天 Bid 测试或暂停";
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  const initialAnalysis=readClientCache<AdAnalysis>(`ads:v6:${initial.start}:${initial.end}`,CLIENT_CACHE_RETENTION_MS);
  ...
  async function queueZombieAction(row:ZombieCampaignFinding){const key=`${row.campaignId}:${row.listing}`;setQueueState(value=>({...value,[key]:'saving'}));setBatchMessage('');try{const response=await fetch('/api/ads/actions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({runKey:data?.runKey,listing:row.listing,campaignId:row.campaignId,actionType:row.actionType,before:row.before,proposed:row.proposed})});const body=await response.json() as {message?:string;error?:string};if(!response.ok)throw new Error(body.error||'执行单保存失败');await reloadQueue();setBatchMessage('僵尸 Campaign 诊断已加入本周执行批次，等待人工处理。');}catch(reason){setQueueState(value=>({...value,[key]:reason instanceof Error?reason.message:'保存失败'}));}}
  ...
  async function prepareSelected(){const actions=queuedActions.filter(action=>selectedQueue.includes(action.id)&&isBulkApprovable(action));if(!actions.length&&!approvedActions.length)return;setBatchBusy(true);setBatchMessage('');try{await Promise.all(actions.map(async action=>{const response=await fetch('/api/ads/actions',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({id:action.id,status:'APPROVED'})});const body=await response.json() as {error?:string};if(!response.ok)throw new Error(`${action.listing}: ${body.error||'确认失败'}`);}));const body=await postExecution(true);setSelectedQueue([]);setBatchMessage(body.message||`已确认并预检 ${actions.length+approvedActions.length} 项。`);}catch(reason){setBatchMessage(reason instanceof Error?reason.message:'确认并预检失败');}finally{setBatchBusy(false);}}
  ...
  const aiDiagnostics=data?.aiCampaignDiagnostics||[];
  const aiAdEligibility=data?.aiAdEligibility||[];
  const aiLaunchReady=aiAdEligibility.filter(item=>item.canLaunch).length;
  const urgentAiDiagnostics=aiDiagnostics.filter(item=>item.priority==='P0').length;
  ...
  const approvableQueue=filteredQueue.filter(action=>isBulkApprovable(action));
  ...
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{requested.start} 至 {requested.end}</span></div>} />
    ...
    <section className="card ai-campaign-diagnostics"><div className="section-head"><div><span>PRE-LAUNCH GATE</span><h2>AI 广告新开评估</h2></div><b>{aiLaunchReady?`${aiLaunchReady} 个SKU满足准入`:'本期不建议新开'}</b></div>
      ...
      <div className="ai-diagnosis-list">{aiAdEligibility.slice(0,5).map(item=><article className={`ai-diagnosis-card ${item.canLaunch?'learned':item.status==='BLOCKED'?'learning_overdue':'learning'}`} key={item.listing}><header><div><em>{item.canLaunch?'READY':item.status==='BLOCKED'?'BLOCK':'WAIT'}</em><strong>{item.listing}</strong><small>Campaign {item.campaignId} · {item.parts.join(' / ')||'Part未映射'}</small></div><span>{item.canLaunch?'可在下一周期启动':item.status==='BLOCKED'?'先修复阻塞':'订单量不足'}</span></header><div className="ai-diagnosis-metrics"><p><span>14天归因订单</span><b>{item.orders14d} / 50</b></p><p><span>达标缺口</span><b>{item.orderGap} 单</b></p><p><span>14天花费 / ROAS</span><b>{money2(item.spend14d)} / {item.wscRoas14d.toFixed(2)}×</b></p><p><span>库存覆盖</span><b>{item.inventory.known?`${Math.round(item.inventory.coverDays||0)}天`:'未知'}</b></p></div><p className="ai-diagnosis-summary">{item.whenToLaunch}</p><div className="ai-diagnosis-action"><b>{item.canLaunch?`开前Cap ${money2(item.preLaunchDailyCap)} · tROAS ≥${item.targetRoasFloor}%`:'继续 Manual 积累'}</b><span>{item.guardrail}</span>{item.blockers.length?<em>{item.blockers.join('；')}</em>:null}</div></article>)}{!loading&&!aiAdEligibility.length?<p className="empty-state">暂无可评估的 Manual Campaign SKU。</p>:null}</div>
    </section>
    <section className="card ai-campaign-diagnostics"><div className="section-head"><div><span>CAMPAIGN-LEVEL GUARDRAIL</span><h2>AI Campaign 学习诊断</h2></div><b>门槛：14天内50个归因订单</b></div>
      <div className="ai-diagnosis-list">{aiDiagnostics.map(item=><article className={`ai-diagnosis-card ${item.stage.toLowerCase()}`} key={item.campaignId}><header><div><em>{item.priority}</em><strong>{item.name||`Campaign ${item.campaignId}`}</strong><small>ID {item.campaignId} · 开始 {item.startDate||'未知'}{item.statusConflict?` · 状态冲突：${item.platformSource} ${item.platformStage} / 报表 ${item.reportedStatus}`:''}</small></div><span>{item.statusConflict?'状态冲突':item.stage==='LEARNING_OVERDUE'?'学习期异常':item.stage==='LEARNING'?'学习中':item.stage==='LEARNED'?'学习完成':'已暂停'}</span></header><div className="ai-diagnosis-metrics"><p><span>14天归因订单</span><b>{item.orders14d} / 50</b></p><p><span>14天花费</span><b>{money2(item.current14d.spend)}</b></p><p><span>Target ROAS</span><b>{item.targetRoas?`${item.targetRoas}%`:'—'}</b></p><p><span>Daily Cap</span><b className={item.noDailyCap?'bad':''}>{adBudgetValue(item.dailyCap)}</b></p></div><p className="ai-diagnosis-summary">{item.summary}</p><div className="ai-diagnosis-action"><b>{item.action==='CONTACT_ACCOUNT_MANAGER'?'联系 Account Manager':item.action==='WAIT_FOR_LEARNING'?'等待学习完成':item.action==='REVIEW_TARGETS'?'进入人工目标复盘':'保持暂停'}</b><span>{item.guardrail}</span>{item.statusConflict&&<em>平台状态证据：{item.platformObservedAt||'日期未知'}，需与最新 Campaign 报表人工对账</em>}{item.noDailyCap&&<em>无Daily Cap：记录为花费风险，不在学习期内自动修改</em>}</div></article>)}{!loading&&!aiDiagnostics.length?<p className="empty-state">当前没有AI-TROAS Campaign。</p>:null}</div>
    </section>
    <section className="card ai-campaign-diagnostics"><div className="section-head"><div><span>DELIVERY &amp; LINK AUDIT</span><h2>僵尸 Campaign 规则</h2></div><b>{zombieAudit.matureDays} 个成熟日 · 硬僵尸 {zombieAudit.hard} · 准僵尸 {zombieAudit.near}</b></div>
      <div className="ai-diagnosis-list">{zombieFindings.map(item=>{const key=`${item.campaignId}:${item.listing}`;const queued=queueState[key];return <article className={`ai-diagnosis-card ${item.severity==='P0'?'learning_overdue':'learning'}`} key={`${key}:${item.actionType}`}><header><div><em>{item.severity}</em><strong>{item.campaignName||`Campaign ${item.campaignId}`}</strong><small>ID {item.campaignId} · {item.targetingType||'Targeting 未知'} · {item.listing}</small></div><span>{item.severity==='P0'?'硬僵尸':'准僵尸'}</span></header><div className="ai-diagnosis-metrics"><p><span>成熟期曝光</span><b>{item.metric.impressions}</b></p><p><span>成熟期花费</span><b>{money2(item.metric.spend)}</b></p><p><span>归因订单</span><b>{item.metric.orders}</b></p><p><span>链接 / Bid</span><b>{item.linkStatus} / {money2(item.bid)}</b></p></div><p className="ai-diagnosis-summary">{item.label}</p><div className="ai-diagnosis-action"><b>人工执行项</b><span>{item.reasons.join('；')}</span><button disabled={Boolean(queued)} onClick={()=>queueZombieAction(item)}>{queued==='saving'?'保存中':queued?'已加入执行清单':'加入 AI 执行清单'}</button></div></article>})}{!loading&&!zombieFindings.length?<p className="empty-state">未发现满足规则的硬僵尸或准僵尸 Campaign。</p>:null}</div>
    </section>
    <section className="stat-grid four ad-kpis">{[
      ...
      [loading?'-':String(ready),"本周调整建议",`${needsInput} 项预算待审批，${noChange} 项保持 · BM CPC锚`],
    ].map(([value,label,note])=><article className="stat" key={label}><strong>{value}</strong><span>{label}</span><small>{note}</small></article>)}</section>
    <section className="card queue-panel"><div className="section-head"><div><span>执行批次</span><h2>本周执行批次</h2></div><b>{queueLoading?'读取中':`${queuedActions.length} 项 · 成功 ${executedActions.length} · 失败 ${failedActions.length} · 待执行 ${validatedActions.length}`}</b></div>
      ...
      <div className="queue-rows"><div className="queue-row queue-head"><input type="checkbox" aria-label="选择全部可确认执行项" checked={Boolean(approvableQueue.length)&&approvableQueue.every(action=>selectedQueue.includes(action.id))} onChange={event=>setSelectedQueue(event.target.checked?approvableQueue.map(action=>action.id):[])}/><span>Listing</span><span>修改内容</span><span>状态</span><span>执行结果</span><span>操作</span></div>{filteredQueue.map(action=>{const apiAction=['SET_LISTING_BID','SET_LISTING_ACTIVE'].includes(action.action_type);const selectable=isBulkApprovable(action)&&apiAction;const result=executionResultForAction(action);return <article className="queue-row" key={action.id}><input type="checkbox" aria-label={`选择 ${action.listing}`} disabled={!selectable} checked={selectedQueue.includes(action.id)} onChange={event=>setSelectedQueue(value=>event.target.checked?[...value,action.id]:value.filter(id=>id!==action.id))}/><div><strong>{action.listing}</strong><small>Campaign {action.campaign_id}</small></div><b>{actionPayloadText(action)}</b><span className={`queue-status ${action.status.toLowerCase()}`}>{action.status==='PLANNED'?apiAction?'待确认':'人工执行项':action.status==='APPROVED'?'待预检':action.status==='VALIDATED'?'预检通过':action.status==='EXECUTING'?'正在执行':action.status==='EXECUTED'?'已执行':'失败，可移除或重试'}</span><div className={`queue-result ${result.tone}`}><b>{result.title}</b><small>{result.detail}</small></div><div className="queue-actions">{canRemoveAction(action.status)?<button disabled={batchBusy} onClick={()=>removeAction(action)}>移除</button>:null}{action.status==='FAILED'&&apiAction?<small>勾选后重新预检</small>:null}{!apiAction?<small>Campaign / 资格动作需人工处理，不进入批量写入</small>:null}</div></article>})}{!queueLoading&&!filteredQueue.length?<p className="empty-state">没有符合筛选条件的执行项。</p>:null}</div>
