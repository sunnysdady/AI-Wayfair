...
function Dashboard() {
  ...
  async function syncAllSites() {
    ...
    try {
      ...
      const body = await response.json() as { error?: string; catalog?: { continued?: boolean } };
      if (!response.ok) throw new Error(body.error || "全站点同步失败");
      setError(body.catalog?.continued ? "全站点同步已启动；Catalog 正在分批续传" : "全站点数据已同步");
      refreshDashboard();
    } catch (reason) {
  ...
  return <>
    ...
    <section className="date-console" aria-label="经营周期">
      ...
      <div className="dashboard-sync-tools"><span className={error ? "sync-state error" : "sync-state"}>{error || (data?.sync.stale ? `同步失败，显示缓存：${data.sync.error}` : data?.sync.syncedAt ? `最近同步 ${new Date(data.sync.syncedAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}` : "正在连接订单数据")}</span><button type="button" className={`dashboard-refresh dashboard-full-sync ${syncingAll?"refreshing":""}`} aria-label="同步全站点数据" title="同步全站点数据" disabled={syncingAll} onClick={syncAllSites}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8 8 0 1 0-2.34 5.66M20 4v7h-7"/></svg><span>{syncingAll ? "同步中…" : "同步全站点"}</span></button></div>
    </section>
...
function Inventory({ embedded = false }: { embedded?: boolean }) {
  ...
  const [file,setFile]=useState<File|null>(null);const [preview,setPreview]=useState<Preview|null>(savedPreview);const [state,setState]=useState(savedPreview?.snapshotId?'最近快照可用':"读取最近快照");const [busy,setBusy]=useState(false);const [zeroConfirmed,setZeroConfirmed]=useState(false);const [message,setMessage]=useState("");
