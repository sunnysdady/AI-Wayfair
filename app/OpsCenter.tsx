...
type AdAnalysis = {
  current: AdMetric; previous: AdMetric; history: ({ date: string } & AdMetric)[]; campaigns: AdCampaign[]; listings: AdListing[]; aiCampaignDiagnostics: AiCampaignDiagnosis[]; zombieFindings: ZombieCampaignFinding[];
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  const initialAnalysis=readClientCache<AdAnalysis>(`ads:v5:${initial.start}:${initial.end}`,CLIENT_CACHE_RETENTION_MS);
