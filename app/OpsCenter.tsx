...
function Dashboard() {
  ...
  const [emailBrief,setEmailBrief]=useState<EmailBrief|null>(null);
  ...
  useEffect(()=>{const controller=new AbortController();fetch('/api/email/daily',{signal:controller.signal}).then(async response=>{const body=await response.json() as EmailBrief;if(!response.ok)throw new Error(body.error||'Outlook 日报读取失败');return body;}).then(setEmailBrief).catch(()=>{});return()=>controller.abort();},[]);
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  const [historyWeeks,setHistoryWeeks]=useState<AdHistoryWeek[]>([]);
  ...
  useEffect(()=>{if(tab!=='ai')return;const controller=new AbortController();fetch('/api/ads/history',{signal:controller.signal}).then(async response=>{const body=await response.json() as {weeks?:AdHistoryWeek[]};if(response.ok)setHistoryWeeks(body.weeks||[]);}).catch(()=>{});return()=>controller.abort();},[tab,data?.runKey,queuedActions.length]);
  ...
  const approvableQueue=filteredQueue.filter(action=>action.action_type==='SET_LISTING_BID'&&isBulkApprovable(action)&&(action.status==='FAILED'||data?.listings.find(row=>row.campaignId===action.campaign_id&&row.listing===action.listing)?.action.execution==='READY_FOR_PLAN'));
