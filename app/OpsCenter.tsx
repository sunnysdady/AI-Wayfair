...
import { filterAdActions, isBulkApprovable, queuedActionState } from "../lib/ad-action-queue.mjs";

type View = "daily" | "plan" | "inventory" | "ads" | "review" | "catalog" | "sources" | "help";
...
function ShellHeader({ active, onNavigate }: { active: View; onNavigate: (view: View) => void }) {
  return <aside className="sidebar">
    ...
    <nav className="nav" aria-label="主导航">
      {NAV.map((item) => <button key={item.id} className={active === item.id ? "active" : ""} onClick={() => onNavigate(item.id)}>{item.label}</button>)}
    </nav>
...
function Daily() {
  ...
  return <>
    ...
    <section className="card order-performance">
      <div className="section-head"><div><span>订单业绩</span><h2>{rangeLabel} 订单走势</h2></div><b>{data?.sync.refreshed ? "API 已刷新并写入缓存" : "15 分钟缓存，增量预载"}</b></div>
    ...
    <div className="daily-grid">
      <article className="card feature-card">
        <span className="pill amber">需尽快处理</span>
...
function Ads() {
  ...
  const [queuedActions,setQueuedActions]=useState<QueuedAdAction[]>([]); const [queueLoading,setQueueLoading]=useState(false); const [queueError,setQueueError]=useState('');
  const [liveEnabled,setLiveEnabled]=useState(false); const [batchBusy,setBatchBusy]=useState(false); const [batchMessage,setBatchMessage]=useState(''); const [adConfirmation,setAdConfirmation]=useState('');
  ...
  useEffect(()=>{if(!data?.runKey)return;const controller=new AbortController();setQueueLoading(true);setQueueError('');setQueueState({});setQueuedActions([]);fetch(`/api/ads/actions?runKey=${encodeURIComponent(data.runKey)}`,{signal:controller.signal}).then(async response=>{const body=await response.json() as {actions?:QueuedAdAction[];liveEnabled?:boolean;error?:string};if(!response.ok)throw new Error(body.error||'执行批次读取失败');return body;}).then(body=>{const actions=body.actions||[];setQueuedActions(actions);setQueueState(queuedActionState(actions));setLiveEnabled(Boolean(body.liveEnabled));}).catch(reason=>{if(reason.name!=='AbortError')setQueueError(reason.message||'执行批次读取失败');}).finally(()=>{if(!controller.signal.aborted)setQueueLoading(false);});return()=>controller.abort();},[data?.runKey]);
  ...
  async function reloadQueue(){if(!data?.runKey)return;const response=await fetch(`/api/ads/actions?runKey=${encodeURIComponent(data.runKey)}`);const body=await response.json() as {actions?:QueuedAdAction[];liveEnabled?:boolean;error?:string};if(!response.ok)throw new Error(body.error||'执行批次读取失败');const actions=body.actions||[];setQueuedActions(actions);setQueueState(queuedActionState(actions));setLiveEnabled(Boolean(body.liveEnabled));}
  ...
  async function approveAction(action:QueuedAdAction){setBatchBusy(true);setBatchMessage('');try{const response=await fetch('/api/ads/actions',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({id:action.id,status:'APPROVED'})});const body=await response.json() as {message?:string;error?:string};if(!response.ok)throw new Error(body.error||'执行项确认失败');await reloadQueue();setBatchMessage(body.message||'已确认进入 API 预检。');}catch(reason){setBatchMessage(reason instanceof Error?reason.message:'确认失败');}finally{setBatchBusy(false);}}
  ...
  async function executeBatch(dryRun:boolean){if(!data?.runKey)return;setBatchBusy(true);setBatchMessage('');try{const response=await fetch('/api/ads/actions/execute',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({runKey:data.runKey,dryRun,confirmation:adConfirmation})});const body=await response.json() as {message?:string;error?:string};if(!response.ok)throw new Error(body.error||'广告执行失败');await reloadQueue();setBatchMessage(body.message||'执行完成。');}catch(reason){setBatchMessage(reason instanceof Error?reason.message:'广告执行失败');}finally{setBatchBusy(false);}}
  ...
  async function approveSelected(){const actions=queuedActions.filter(action=>selectedQueue.includes(action.id)&&isBulkApprovable(action));if(!actions.length)return;setBatchBusy(true);setBatchMessage('');try{await Promise.all(actions.map(async action=>{const response=await fetch('/api/ads/actions',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({id:action.id,status:'APPROVED'})});const body=await response.json() as {error?:string};if(!response.ok)throw new Error(`${action.listing}: ${body.error||'确认失败'}`);}));await reloadQueue();setSelectedQueue([]);setBatchMessage(`已确认 ${actions.length} 项，可执行 API 预检。`);}catch(reason){setBatchMessage(reason instanceof Error?reason.message:'批量确认失败');}finally{setBatchBusy(false);}}
  ...
  const plannedActions=queuedActions.filter(item=>item.status==='PLANNED'); const approvedActions=queuedActions.filter(item=>item.status==='APPROVED'); const validatedActions=queuedActions.filter(item=>item.status==='VALIDATED'); const executedActions=queuedActions.filter(item=>item.status==='EXECUTED');
  ...
  const approvableQueue=filteredQueue.filter(action=>isBulkApprovable(action)&&(action.status==='FAILED'||data?.listings.find(row=>row.campaignId===action.campaign_id&&row.listing===action.listing)?.action.execution==='READY_FOR_PLAN'));
  return <><Hero eyebrow="" title="广告周优化" text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':`${ready} 项待处理`}</b><span>{data?.decisionRange.start||'-'} 至 {data?.decisionRange.end||'-'}</span></div>} />
    ...
    <section className="stat-grid four ad-kpis">{[
      ...
      [loading?'-':String(ready),"本周调整建议",`${needsInput} 项需补条件，${noChange} 项保持`],
    ].map(([value,label,note])=><article className="stat" key={label}><strong>{value}</strong><span>{label}</span><small>{note}</small></article>)}</section>
    <section className="card queue-panel"><div className="section-head"><div><span>执行批次</span><h2>本周执行批次</h2></div><b>{queueLoading?'读取中':`${queuedActions.length} 项，待确认 ${plannedActions.length}，已预检 ${validatedActions.length}，已执行 ${executedActions.length}`}</b></div>
      <div className="table-tools queue-tools"><label>状态<select value={queueStatusFilter} onChange={event=>setQueueStatusFilter(event.target.value)}><option value="ALL">全部</option><option value="PLANNED">待确认</option><option value="FAILED">执行失败</option><option value="APPROVED">待预检</option><option value="VALIDATED">预检通过</option><option value="EXECUTED">已执行</option></select></label><span>已选 {selectedQueue.length} 项</span><button disabled={!selectedQueue.length||batchBusy} onClick={approveSelected}>批量确认或重试</button></div>
      {queueError&&<div className="inline-error">{queueError}</div>}
      <div className="queue-rows"><div className="queue-row queue-head"><input type="checkbox" aria-label="选择全部可确认执行项" checked={Boolean(approvableQueue.length)&&approvableQueue.every(action=>selectedQueue.includes(action.id))} onChange={event=>setSelectedQueue(event.target.checked?approvableQueue.map(action=>action.id):[])}/><span>Listing</span><span>修改内容</span><span>状态</span><span>操作</span></div>{filteredQueue.map(action=>{const source=data?.listings.find(row=>row.campaignId===action.campaign_id&&row.listing===action.listing);const apiAction=['SET_LISTING_BID','SET_LISTING_ACTIVE'].includes(action.action_type);const confirmable=apiAction&&source?.action.execution==='READY_FOR_PLAN';const selectable=isBulkApprovable(action)&&(action.status==='FAILED'||confirmable);return <article className="queue-row" key={action.id}><input type="checkbox" aria-label={`选择 ${action.listing}`} disabled={!selectable} checked={selectedQueue.includes(action.id)} onChange={event=>setSelectedQueue(value=>event.target.checked?[...value,action.id]:value.filter(id=>id!==action.id))}/><div><strong>{action.listing}</strong><small>Campaign {action.campaign_id}</small></div><b>{actionPayloadText(action)}</b><span className={`queue-status ${action.status.toLowerCase()}`}>{action.status==='PLANNED'?apiAction?'待确认':'人工任务':action.status==='APPROVED'?'待预检':action.status==='VALIDATED'?'预检通过':action.status==='EXECUTING'?'正在执行':action.status==='EXECUTED'?'已执行':'失败，可重试'}</span><div>{selectable?<button disabled={batchBusy} onClick={()=>approveAction(action)}>{action.status==='FAILED'?'重试':'确认'}</button>:null}{action.status==='PLANNED'?<button disabled={batchBusy} onClick={()=>removeAction(action)}>移除</button>:null}{!confirmable&&action.status==='PLANNED'&&apiAction?<small>{source?.action.blockers.join('；')||'当前周期不满足执行条件'}</small>:null}</div></article>})}{!queueLoading&&!filteredQueue.length?<p className="empty-state">没有符合筛选条件的执行项。</p>:null}</div>
      <div className="batch-controls"><div><b>API 执行控制</b><small>只执行 Listing Bid/启停；Campaign Cap、tROAS、关键词仍是 Partner Home 人工任务。</small></div><button className="primary" disabled={!approvedActions.length||batchBusy} onClick={()=>executeBatch(true)}>执行 API Dry-run 预检</button><label>正式确认<input value={adConfirmation} onChange={event=>setAdConfirmation(event.target.value)} placeholder="输入：执行广告修改"/></label><button className="primary dark" disabled={!validatedActions.length||batchBusy||!liveEnabled||adConfirmation!=='执行广告修改'} onClick={()=>executeBatch(false)}>正式执行 Wayfair 广告修改</button><em>{liveEnabled?'生产写入已启用':'生产写入安全开关未启用'}</em></div>
      {batchMessage&&<div className={/(失败|未启用|必须)/.test(batchMessage)?'inventory-message bad':'inventory-message good'}>{batchMessage}</div>}
    </section>
...
function Help() {
  return <><Hero eyebrow="" title="帮助" text="" />
    <div className="help-grid">
      <section className="card help-section"><h2>广告优化</h2><dl><div><dt>调整频率</dt><dd>每周生成一次建议，固定读取截至 T-14 的最近 7 天成熟数据。</dd></div><div><dt>建议依据</dt><dd>历史表现、SKU 利润、链接质量、库存覆盖和月度计划共同判断。</dd></div><div><dt>执行范围</dt><dd>Listing Bid 与启停支持 API。Campaign Cap、tROAS、关键词和否词保留为人工任务。</dd></div></dl></section>
...
export default function OpsCenter() {
  ...
  const page=useMemo(()=>({daily:<Daily/>,plan:<Plan onNavigate={setView}/>,inventory:<Inventory/>,ads:<Ads/>,review:<Review/>,catalog:<Catalog/>,sources:<Sources/>,help:<Help/>})[view],[view]);
