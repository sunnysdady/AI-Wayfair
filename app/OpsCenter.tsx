...
type PlanProgress = {
  ...
  nextPlan: {
    ...
    promotionEvents: { id: string; name: string; status: "OPEN_FOR_SUBMISSION"; planningStatus: "UPCOMING_SUBMISSION" | "MISSED_DEADLINE_RECHECK"; canRelyOnForPlan: boolean; planningNote: string; submissionOpened: string; curationDeadline: string; start: string; end: string; lengthDays: number; category: string; recommendedProducts: number | null; sourceAsOf: string }[];
    promotionPlan: { listing: string; part: string; role: "VOLUME_CORE" | "PROFIT_POOL" | "CONTROLLED_GROWTH" | "REPAIR_ORGANIC"; action: "PROPOSE" | "HOLD"; priceBasisCents: number | null; costCents: number | null; inventoryOnHand: number; catalogLiveCount: number; eventIds: string[]; discountPlan: string; b2cDiscount: number; b2bTotalDiscount: number; quantityOffer: number; worstDiscount: number; estimatedWorstMargin: number | null; roleMarginFloor: number; reason: string; requiredGates: string[]; reviewStatus: "PENDING_REVIEW"; canSubmitToZiniao: false }[];
    promotionPortfolio: { originalAdBudget: number; recommendedAdBudget: number; baseAdBudget: number; performanceReserve: number; fallbackAdBudget: number; methodology: string; budgetRule: string; scenarios: { promotionOrderShare: number; projectedRevenue: number; projectedGrossProfit: number; projectedPostAdProfit: number; projectedPostAdMargin: number; hardAdCapAt10Percent: number; targetAdCapAt12Percent: number }[] };
    promotionSummary: { totalListings: number; totalParts: number; proposedListings: number; proposedParts: number; heldParts: number; pendingReviewParts: number; ziniaoReadyParts: number; submissionLocked: boolean; originalAdBudget: number; recommendedAdBudget: number; adBudgetReduction: number; projectedPromotionOrderShare: number; projectedRevenue: number; projectedPostAdProfit: number; projectedPostAdMargin: number; stressPromotionOrderShare: number; stressPostAdMargin: number; fallbackAdBudget: number; fullPromotionHardAdCap: number };
  };
  error?: string;
};
...
function promotionEventShortName(id: string) {
  const labels: Record<string,string> = {
    "member-pop-up-august-2026": "Member Pop Up",
    "summer-markdowns-august-2026": "Summer",
    "four-day-flash-august-2026": "Flash",
    "labor-day-august-2026": "Labor Day",
  };
  return labels[id] || id;
}
...
function Plan({ embedded = false, onOpenReview, tab, onTabChange }: { embedded?: boolean; onOpenReview: () => void; tab: PlanSection; onTabChange: (tab: PlanSection) => void }) {
  ...
  return <>{!embedded&&<Hero eyebrow="MONTHLY OPERATING PLAN" title="目标与执行" text="6月复盘 → 7月真实基线执行 → 8月下一阶段准备；目标、利润与广告共用同一套运营计划" side={<button className="hero-button" onClick={onOpenReview}>查看完整复盘证据</button>} />}
    ...
    {tab==='august'&&<><section className="august-sales-summary">{[
      ...
      [money(promotionSummary?.recommendedAdBudget||0),'促销兼容广告预算',`原销售预算 ${money(promotionSummary?.originalAdBudget||salesSummary?.plannedAdBudget||0)} · 待促销审核`],
    ...
    <div className="plan-workspace august-order-workspace">
      ...
      <section className="card promotion-calendar"><div className="section-head"><div><span>PARTNER HOME · 2026-07-28</span><h2>8月活动清单</h2></div><b>4 个可规划 · 2 个截止已过待复核</b></div><div className="promotion-event-grid">{(data?.nextPlan.promotionEvents||[]).map(event=><article key={event.id}><header><span>{event.category}</span><b className={event.planningStatus==='MISSED_DEADLINE_RECHECK'?'missed':''}>{event.planningStatus==='MISSED_DEADLINE_RECHECK'?'截止已过 · 复核补报':'可规划'}</b></header><h3>{event.name}</h3><dl><div><dt>提报截止</dt><dd>{event.curationDeadline}</dd></div><div><dt>活动周期</dt><dd>{event.start}<br/>{event.end}</dd></div><div><dt>运营判断</dt><dd>{event.planningNote}</dd></div></dl><footer><span>{event.lengthDays} 天</span><small>{event.recommendedProducts?`${event.recommendedProducts} 个后台推荐商品`:'后台未显示推荐数'}</small></footer></article>)}</div></section>
      <section className="card promotion-review"><div className="section-head"><div><span>PENDING PROMOTION REVIEW · ZINIAO LOCKED</span><h2>逐Part促销审核方案</h2></div><b className="promotion-lock">待你审核 · 紫鸟锁定</b></div>
        <div className="promotion-review-summary"><article><span>建议提报</span><strong>{promotionSummary?.proposedParts||0} Parts</strong><small>{promotionSummary?.proposedListings||0} 个Listing</small></article><article><span>暂缓</span><strong>{promotionSummary?.heldParts||0} Parts</strong><small>缺价 / 缺成本 / 低库存 / 重复映射</small></article><article><span>促销兼容广告预算</span><strong>{money(promotionSummary?.recommendedAdBudget||0)}</strong><small>基础 {money(data?.nextPlan.promotionPortfolio.baseAdBudget||0)} + 赢家 {money(data?.nextPlan.promotionPortfolio.performanceReserve||0)}</small></article><article><span>紫鸟可提报</span><strong>{promotionSummary?.ziniaoReadyParts||0}</strong><small>审核前 canSubmit=false</small></article></div>
        <div className="promotion-budget-warning"><b>{money(promotionSummary?.originalAdBudget||0)} → {money(promotionSummary?.recommendedAdBudget||0)}</b><span>原销售预算未计促销折扣。主情景按 {percent(promotionSummary?.projectedPromotionOrderShare||0)} 促销订单占比，广告后利润率 {percent(promotionSummary?.projectedPostAdMargin||0)}；70%压力情景 {percent(promotionSummary?.stressPostAdMargin||0)}。超过70%或后台压价低于模型，广告收紧至 {money(promotionSummary?.fallbackAdBudget||0)}；100%促销极端情景的10%硬上限为 {money(promotionSummary?.fullPromotionHardAdCap||0)}。</span></div>
        <div className="promotion-review-table"><div className="promotion-review-row head"><span>Listing / Part</span><span>角色 / 证据</span><span>折扣与理由</span><span>最差毛利</span><span>活动 / 决策</span></div>{(data?.nextPlan.promotionPlan||[]).map(item=><article className="promotion-review-row" key={`${item.listing}-${item.part}`}><span><b>{item.listing}</b><small>{item.part}</small></span><span><b className={`sales-role ${item.role.toLowerCase()}`}>{salesRoleNames[item.role]||item.role}</b><small>价 {item.priceBasisCents==null?'待补':money2(item.priceBasisCents/100)} · 成本 {item.costCents==null?'待补':money2(item.costCents/100)} · 库存 {item.inventoryOnHand} · LIVE映射 {item.catalogLiveCount}</small></span><span><b>{item.discountPlan}</b><small>{item.reason}</small></span><span><b>{item.estimatedWorstMargin==null?'待补证据':percent(item.estimatedWorstMargin)}</b><small>角色底线 {percent(item.roleMarginFloor)}</small></span><span><b className={`promotion-decision ${item.action.toLowerCase()}`}>{item.action==='PROPOSE'?'建议提报':'暂缓'}</b><small>{item.eventIds.length?item.eventIds.map(promotionEventShortName).join(' · '):'不进入活动'}</small></span></article>)}</div>
        <footer className="promotion-review-note"><b>提交边界：</b> 当前全部 {promotionSummary?.pendingReviewParts||0} 个Part仍为 PENDING_REVIEW，紫鸟可提报为0。审核通过后还要在Wayfair后台复核推荐资格、实时接受价、库存与折扣叠加，再生成紫鸟提报清单。</footer>
      </section></>}
  </>;
}
