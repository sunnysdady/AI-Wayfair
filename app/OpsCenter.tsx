...
type PlanProgress = {
  ...
  nextPlan: {
    ...
    promotionPlan: { listing: string; part: string; role: "VOLUME_CORE" | "PROFIT_POOL" | "CONTROLLED_GROWTH" | "REPAIR_ORGANIC"; action: "SUBMITTED" | "HOLD"; priceBasisCents: number | null; costCents: number | null; inventoryOnHand: number; catalogLiveCount: number; eventIds: string[]; activeEventIds: string[]; submittedEventIds: string[]; discountPlan: string; b2cDiscount: number; b2bTotalDiscount: number; memberB2cDiscount: number | null; quantityOffer: number; quantityPromotionStatus: "PROCESSING" | "NOT_APPLICABLE"; worstDiscount: number; estimatedWorstMargin: number | null; roleMarginFloor: number; marginAlert: boolean; reason: string; requiredGates: string[]; reviewStatus: "APPROVED" | "APPROVED_HOLD"; submittedToZiniao: boolean; canSubmitToZiniao: false }[];
...
function Plan({ embedded = false, onOpenReview, tab, onTabChange }: { embedded?: boolean; onOpenReview: () => void; tab: PlanSection; onTabChange: (tab: PlanSection) => void }) {
  ...
  return <>{!embedded&&<Hero eyebrow="MONTHLY OPERATING PLAN" title="目标与执行" text="6月复盘 → 7月真实基线执行 → 8月下一阶段准备；目标、利润与广告共用同一套运营计划" side={<button className="hero-button" onClick={onOpenReview}>查看完整复盘证据</button>} />}
    ...
    {tab==='august'&&<><section className="august-sales-summary">{[
      [`${salesSummary?.targetOrders||150} Orders`,'8月销量目标',`7月可承接SKU实际 ${data?.nextPlan.salesPlan.baselineMappedOrders||48} 单 · 月末预测 ${data?.nextPlan.salesPlan.currentForecastOrders||56.5} 单`],
      [money(promotionSummary?.projectedRevenue||salesSummary?.projectedRevenue||0),'促销后预计WSC销售额',`按 ${percent(promotionSummary?.projectedPromotionOrderShare||0.6)} 订单命中最差折扣建模`],
      [`${percent(promotionSummary?.projectedPostAdMargin||0)}`,'促销后广告后利润率',`预计贡献 ${money(promotionSummary?.projectedPostAdProfit||0)} · 硬底线 10%`],
      [money(promotionSummary?.recommendedAdBudget||0),'促销兼容广告预算',`原销售预算 ${money(promotionSummary?.originalAdBudget||salesSummary?.plannedAdBudget||0)} · 已计活动与多件折扣`],
    ].map(([value,label,note])=><article className="card" key={label}><strong>{value}</strong><span>{label}</span><small>{note}</small></article>)}</section>
    <div className="plan-workspace august-order-workspace">
      <article className="card target-card"><div className="section-head"><div><span>利润款 + 跑量款组合</span><h2>SKU销量、利润与促销责任</h2></div><b className="plan-approved">销售计划已确认 · 促销已同步</b></div><div className="sales-plan-table"><div className="sales-plan-row head"><span>Listing / Part</span><span>真实基线</span><span>8月订单</span><span>角色</span><span>收入 / 毛利</span><span>促销同步</span><span>广告池</span><span>执行Gate / 止损</span></div>{(data?.nextPlan.salesPlanRows||[]).map(item=><article className="sales-plan-row" key={item.listing}><span><b>{item.listing}</b><small>{item.parts.join(' · ')}</small></span><span><b>7月 {item.julyOrders} 单</b><small>6月 {item.juneOrders} 单 · 库存 {item.inventoryOnHand}</small></span><span><strong>{item.targetOrders}</strong><small>{item.targetOrders>item.julyOrders?`较7月 +${item.targetOrders-item.julyOrders}`:'守住基线'}</small></span><span><b className={`sales-role ${item.role.toLowerCase()}`}>{salesRoleNames[item.role]||item.role}</b><small>{item.tactic}</small></span><span><b>{money(item.expectedRevenue)}</b><small>广告前 {money(item.expectedGrossProfit)} · {percent(item.preAdMarginRate)}{item.marginMode==='STORE_ESTIMATE'?' 估算':''}</small></span><span><b className={`promotion-sync ${item.promotion.status.toLowerCase()}`}>{salesPromotionStatusNames[item.promotion.status]||item.promotion.status}</b><small>{item.promotion.discountTiers.length?`活动 ${item.promotion.discountTiers.join(' · ')}`:'无活动折扣'} · 生效 {item.promotion.activeEventIds.length} / 已提交 {item.promotion.submittedEventIds.length}{item.promotion.quantityOfferParts.length?` · 买2额外5% ${item.promotion.quantityOfferParts.length}款`:''}{item.promotion.heldParts.length?` · 暂缓 ${item.promotion.heldParts.join('、')}`:''}</small></span><span><b>{money(item.plannedAdBudget)}</b><small>促销后按全店 {money(promotionSummary?.recommendedAdBudget||0)} 总池重分配</small></span><span><b>{item.gate}</b><small>止损：{item.stopRule}</small></span></article>)}</div><footer className="sales-portfolio-note"><b>组合经营规则</b><span>{data?.nextPlan.salesPlan.strategy||'利润款补贴跑量款，按全店利润池管理。'} {data?.nextPlan.salesPlan.riskNote}</span></footer></article>
      <aside className="card milestone-card"><div className="section-head"><div><span>周里程碑</span><h2>150单达成节奏</h2></div></div><div className="milestones">{(data?.nextPlan.salesMilestones||[]).map(item=><div key={item.label}><b>{item.label}<small>{item.range}</small></b><strong>{item.cumulativeOrders} Orders</strong><p>本阶段 {item.weekOrders} 单 · {item.note}</p></div>)}</div></aside>
    </div>
      <section className="card promotion-calendar"><div className="section-head"><div><span>PARTNER HOME + PURPLE BIRD · 2026-07-28</span><h2>8月活动提报状态</h2></div><b>{promotionSummary?.activeEvents||0} 个已生效 · {promotionSummary?.submittedEvents||0} 个已提交</b></div><div className="promotion-event-grid">{(data?.nextPlan.promotionEvents||[]).map(event=><article key={event.id}><header><span>{event.category}</span><b className={event.status==='SUBMITTED'?'processing':''}>{event.status==='ACTIVE'?'已生效':'已提交 · 处理中'}</b></header><h3>{event.name}</h3><dl><div><dt>项目号 / SKU</dt><dd>#{event.projectId} · {event.submittedProducts} 个</dd></div><div><dt>活动周期</dt><dd>{event.start}<br/>{event.end}</dd></div><div><dt>同步情况</dt><dd>{event.planningNote}</dd></div></dl><footer><span>{event.lengthDays} 天</span><small>提报 {event.submittedAt} · 截止 {event.curationDeadline}</small></footer></article>)}</div></section>
      <section className="card promotion-review"><div className="section-head"><div><span>PROMOTION SUBMISSION SYNCED · 2026-07-28</span><h2>逐Part促销执行情况</h2></div><b className="promotion-lock synced">已审核并提报</b></div>
        <div className="promotion-review-summary"><article><span>活动已提报</span><strong>{promotionSummary?.submittedParts||0} Parts</strong><small>{promotionSummary?.submittedListings||0} 个Listing · 紫鸟已提交 {promotionSummary?.ziniaoSubmittedParts||0}</small></article><article><span>证据不足暂缓</span><strong>{promotionSummary?.heldParts||0} Parts</strong><small>缺价 / 缺成本 / 低库存 / 重复映射</small></article><article><span>B2B买2额外5%</span><strong>{promotionSummary?.quantityPromotionParts||0} Parts</strong><small>项目 #{data?.nextPlan.quantityPromotion.projectId||'-'} · {promotionSummary?.quantityPromotionStatus==='PROCESSING'?'处理中':'待确认'}</small></article><article><span>促销兼容广告预算</span><strong>{money(promotionSummary?.recommendedAdBudget||0)}</strong><small>主情景广告后利润率 {percent(promotionSummary?.projectedPostAdMargin||0)}</small></article></div>
        <div className="promotion-budget-warning"><b>{money(promotionSummary?.originalAdBudget||0)} → {money(promotionSummary?.recommendedAdBudget||0)}</b><span>已把活动折扣和买2件额外5%计入销售计划。主情景：{percent(promotionSummary?.projectedPromotionOrderShare||0)} 活动订单、{percent(promotionSummary?.projectedQuantityOrderShare||0)} 多件订单，广告后利润率 {percent(promotionSummary?.projectedPostAdMargin||0)}；压力情景：{percent(promotionSummary?.stressPromotionOrderShare||0)} / {percent(promotionSummary?.stressQuantityOrderShare||0)}，利润率 {percent(promotionSummary?.stressPostAdMargin||0)}。低于10%立即停止扩量并把广告收紧至 {money(promotionSummary?.fallbackAdBudget||0)}。</span></div>
        <div className="promotion-review-table"><div className="promotion-review-row head"><span>Listing / Part</span><span>角色 / 证据</span><span>活动折扣 / 多件优惠</span><span>叠加后毛利</span><span>活动 / 状态</span></div>{(data?.nextPlan.promotionPlan||[]).map(item=><article className="promotion-review-row" key={`${item.listing}-${item.part}`}><span><b>{item.listing}</b><small>{item.part}</small></span><span><b className={`sales-role ${item.role.toLowerCase()}`}>{salesRoleNames[item.role]||item.role}</b><small>价 {item.priceBasisCents==null?'待补':money2(item.priceBasisCents/100)} · 成本 {item.costCents==null?'待补':money2(item.costCents/100)} · 库存 {item.inventoryOnHand} · LIVE映射 {item.catalogLiveCount}</small></span><span><b>{item.discountPlan}</b><small>{item.memberB2cDiscount?`会员活动 B2C ${percent(item.memberB2cDiscount)} · `:''}{item.quantityOffer?`多件项目处理中 · `:''}{item.reason}</small></span><span><b className={item.marginAlert?'margin-alert':''}>{item.estimatedWorstMargin==null?'待补证据':percent(item.estimatedWorstMargin)}</b><small>{item.marginAlert?'低于角色底线，禁止额外广告扩量':`角色底线 ${percent(item.roleMarginFloor)}`}</small></span><span><b className={`promotion-decision ${item.action.toLowerCase()}`}>{item.action==='SUBMITTED'?'已提报':'暂缓'}</b><small>{item.activeEventIds.length?`Q3已生效 · `:''}{item.submittedEventIds.length?`${item.submittedEventIds.length}个活动已提交`:'不进入活动'}</small></span></article>)}</div>
        <footer className="promotion-review-note"><b>多件优惠：</b> #{data?.nextPlan.quantityPromotion.projectId||'-'}「{data?.nextPlan.quantityPromotion.name||'B2B Buy 2 Save 5%'}」覆盖 {promotionSummary?.quantityPromotionParts||0} 个SKU，状态为处理中；额外5%会与当时最深B2B/活动折扣叠加。当前有 {promotionSummary?.marginAlertParts||0} 个利润预警SKU，保留提报但不允许额外广告扩量。</footer>
      </section></>}
  </>;
}
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  const initialAnalysis=readClientCache<AdAnalysis>(`ads:v10:${initial.start}:${initial.end}`,CLIENT_CACHE_RETENTION_MS);
