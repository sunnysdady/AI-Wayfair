...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  const initialAnalysis=readClientCache<AdAnalysis>(`ads:v2:${initial.start}:${initial.end}`,CLIENT_CACHE_RETENTION_MS);
  ...
  const pageCount=tab==='manager'?`${data?.campaigns.length||0} 个 Campaign`:tab==='listings'?`${data?.listings.length||0} 个 Listing`:optimizationMode==='manual'?`${manualDone.length} / ${MANUAL_AD_TASKS.length} 已完成`:`${ready} 项 AI 建议`;
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{requested.start} 至 {requested.end}</span></div>} />
    ...
    <section className="card ai-execution-boundary"><div><span>CONTROLLED WRITE</span><h2>AI 优化 · 一键执行</h2><p>仅对已通过 Gate 的 Listing Bid 与启停动作执行。系统先 Dry-run 预检，正式写入仍需确认，不跨越关键词和 Campaign 参数边界。</p></div><b>{ready} 项建议待处理</b></section>
