...
const MANUAL_AD_TASKS = [
  { id: "keyword-structure", priority: "P0", group: "关键词布局", title: "重建 Keyword 结构与 Listing 绑定", detail: "把已出单 Search Term 建为 Exact / Phrase，按 Listing 拆组，品牌词、泛词、场景词分层。" },
  { id: "keyword-remap", priority: "P0", group: "关键词布局", title: "重新映射 filing cabinets", detail: "原 DRCI1007 已退出；将已验证搜索词重新匹配 DMOM1021 / DMOM1022 / DMOM1019，完成前预算为 $0。" },
  { id: "keyword-winners", priority: "P1", group: "关键词放量", title: "分别处理 file cabinet 赢家词", detail: "DMOM1019 保持 $0.45 并扩曝光；DMOM1021 由 $0.20 小步测试到 $0.24；累计 30 点击后再按 ROAS 决策。" },
  { id: "keyword-stoploss", priority: "P0", group: "否词 / 止损", title: "执行 20 点击 0 单止损", detail: "先核对是否有对应商品；无对应商品才否词，有对应商品先修 Listing，永不否自有品牌。" },
  { id: "budget-migration", priority: "P1", group: "预算迁移", title: "按 Gate 从 Product 迁移预算", detail: "Product 连续 7 天 WSC ROAS < 3× 或 CPO > $40 时，最多迁移 $150 到已过 Gate 的 Keyword，账户总预算不增加。" },
  { id: "manual-controls", priority: "P1", group: "Campaign 参数", title: "人工调整 Cap / tROAS / B2B", detail: "关键词、否词、Campaign Cap 和 tROAS 保留人工执行；一次只改一个变量并记录回滚值。" },
] as const;
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{requested.start} 至 {requested.end}</span></div>} />
    ...
    {tab==='ai'&&optimizationMode==='manual'&&<div className="manual-optimization-workspace">
      ...
      <section className="card manual-todo-card"><div className="section-head"><div><span>OPERATOR CHECKLIST</span><h2>手动优化 To-Do List</h2></div><b>{manualDone.length} / {MANUAL_AD_TASKS.length} 已完成</b></div><div className="manual-todo-list">{MANUAL_AD_TASKS.map(task=>{const done=manualDone.includes(task.id);return <label className={done?'done':''} key={task.id}><input type="checkbox" checked={done} onChange={()=>toggleManualTask(task.id)}/><span><em>{task.priority}</em><small>{task.group}</small></span><div><strong>{task.title}</strong><p>{task.detail}</p></div></label>})}</div></section>
    </div>}
