...
function Plan({ embedded = false, onOpenReview, tab, onTabChange }: { embedded?: boolean; onOpenReview: () => void; tab: PlanSection; onTabChange: (tab: PlanSection) => void }) {
  ...
  return <>{!embedded&&<Hero eyebrow="MONTHLY OPERATING PLAN" title="目标与执行" text="6月复盘 → 7月真实基线执行 → 8月下一阶段准备；目标、利润与广告共用同一套运营计划" side={<button className="hero-button" onClick={onOpenReview}>查看完整复盘证据</button>} />}
    <section className="context-strip" aria-label="经营月份导航"><button aria-label="打开6月复盘资料" onClick={onOpenReview}><span>复盘月</span><b>2026-06</b><small>经营事实已归档</small></button><button aria-label="查看7月执行计划" className={tab!=='august'?'active':''} onClick={()=>onTabChange('july')}><span>当前经营月</span><b>{data?.currentOperatingMonth.month||'2026-07'} · 128 Orders</b><small>{data?.currentOperatingMonth.note||'真实基线计划读取中'}</small></button><button aria-label="查看8月准备计划" className={tab==='august'?'active':''} onClick={()=>onTabChange('august')}><span>下一计划月</span><b>2026-08 · 150 Orders</b><small>SKU销售责任待审核，不与7月目标混算</small></button></section>
    ...
    {tab==='august'&&<><section className="august-sales-summary">{[
      ...
      [money(salesSummary?.projectedRevenue||0),'预计WSC销售额',`广告前毛利 ${money(salesSummary?.projectedGrossProfit||0)}`],
      [`${percent(salesSummary?.projectedPostAdMargin||0)}`,'广告后店铺利润率',`预计贡献 ${money(salesSummary?.projectedPostAdProfit||0)} · 硬底线 10%`],
      [money(salesSummary?.plannedAdBudget||0),'可用广告预算',`基础 ${money(salesSummary?.baseAdBudget||0)} + 赢家机动 ${money(salesSummary?.performanceReserve||0)}`],
