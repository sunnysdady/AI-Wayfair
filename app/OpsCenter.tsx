...
type AdAnalysis = {
  current: AdMetric; previous: AdMetric; history: ({ date: string } & AdMetric)[]; campaigns: AdCampaign[]; listings: AdListing[]; zombieFindings: ZombieCampaignFinding[];
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  async function queueZombieAction(row:ZombieCampaignFinding){const key=`${row.campaignId}:${row.listing}`;setQueueState(value=>({...value,[key]:'saving'}));setBatchMessage('');try{const response=await fetch('/api/ads/actions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({runKey:data?.runKey,listing:row.listing,campaignId:row.campaignId,actionType:row.actionType,before:row.before,proposed:row.proposed})});const body=await response.json() as {message?:string;error?:string};if(!response.ok)throw new Error(body.error||'执行单保存失败');await reloadQueue();setBatchMessage('僵尸 Campaign 诊断已加入手动执行清单，不会调用 Advertising API。');}catch(reason){setQueueState(value=>({...value,[key]:reason instanceof Error?reason.message:'保存失败'}));}}
  ...
  const apiListings=(data?.listings||[]).filter(row=>API_AD_ACTION_TYPES.has(row.action.type));
  ...
  const manualQueuedActions=queuedActions.filter(action=>!API_AD_ACTION_TYPES.has(action.action_type));
  ...
  const filteredListings=filterAdActions(apiListings,{query:actionQuery,recommendation:recommendationFilter,queue:queueFilter},queueState) as AdListing[];
  const selectableListings=filteredListings.filter(row=>!queueState[`${row.campaignId}:${row.listing}`]);
  const filteredApiQueue=apiQueuedActions.filter(action=>queueStatusFilter==='ALL'||action.status===queueStatusFilter);
  const approvableQueue=filteredApiQueue.filter(action=>isBulkApprovable(action));
  const apiHistoryWeeks=historyWeeks.map(week=>{const actions=week.actions.filter(action=>API_AD_ACTION_TYPES.has(action.action_type));const actionIds=new Set(actions.map(action=>action.id));return {...week,actions,reviews:week.reviews.filter(review=>review.actionId&&actionIds.has(review.actionId))};});
  ...
  const pageCount=tab==='manager'?`${data?.campaigns.length||0} 个 Campaign`:tab==='listings'?`${data?.listings.length||0} 个 Listing`:tab==='manual'?`${manualQueuedActions.length} 项人工清单 · ${manualDone.length} / ${MANUAL_AD_TASKS.length} 已完成`:`${apiQueuedActions.length} 项 API 执行 · ${ready} 项 Listing 建议`;
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{requested.start} 至 {requested.end}</span></div>} />
    ...
    {tab==='manual'&&<div className="manual-optimization-workspace">
      ...
      <section className="card ai-campaign-diagnostics"><div className="section-head"><div><span>MANUAL DIAGNOSIS</span><h2>Campaign / 资格诊断</h2></div><b>{zombieAudit.matureDays} 个成熟日 · 硬僵尸 {zombieAudit.hard} · 准僵尸 {zombieAudit.near}</b></div>
        <p className="allocation-note">这些是平台规则或 Campaign 级诊断，Advertising API 无法安全代执行；确认后加入下方人工清单。</p>
        <div className="ai-diagnosis-list">{zombieFindings.map(item=>{const key=`${item.campaignId}:${item.listing}`;const queued=queueState[key];return <article className={`ai-diagnosis-card ${item.severity==='P0'?'learning_overdue':'learning'}`} key={`${key}:${item.actionType}`}><header><div><em>{item.severity}</em><strong>{item.campaignName||`Campaign ${item.campaignId}`}</strong><small>ID {item.campaignId} · {item.targetingType||'Targeting 未知'} · {item.listing}</small></div><span>{item.severity==='P0'?'硬僵尸':'准僵尸'}</span></header><div className="ai-diagnosis-metrics"><p><span>成熟期曝光</span><b>{item.metric.impressions}</b></p><p><span>成熟期花费</span><b>{money2(item.metric.spend)}</b></p><p><span>归因订单</span><b>{item.metric.orders}</b></p><p><span>链接 / Bid</span><b>{item.linkStatus} / {money2(item.bid)}</b></p></div><p className="ai-diagnosis-summary">{item.label}</p><div className="ai-diagnosis-action"><b>人工执行项</b><span>{item.reasons.join('；')}</span><button disabled={Boolean(queued)} onClick={()=>queueZombieAction(item)}>{queued==='saving'?'保存中':queued?'已加入手动清单':'加入手动执行清单'}</button></div></article>})}{!loading&&!zombieFindings.length?<p className="empty-state">未发现满足规则的硬僵尸或准僵尸 Campaign。</p>:null}</div>
      </section>
      <section className="card queue-panel manual-queue-panel"><div className="section-head"><div><span>NO API WRITE</span><h2>人工执行清单（不调用 API）</h2></div><b>{queueLoading?'读取中':`${manualQueuedActions.length} 项待人工处理`}</b></div>
        {queueError&&<div className="inline-error">{queueError}</div>}
        <div className="queue-rows"><div className="queue-row queue-head"><span>类型</span><span>Listing</span><span>处理内容</span><span>状态</span><span>执行边界</span><span>操作</span></div>{manualQueuedActions.map(action=><article className="queue-row" key={action.id}><em>人工</em><div><strong>{action.listing}</strong><small>Campaign {action.campaign_id}</small></div><b>{actionPayloadText(action)}</b><span className={`queue-status ${action.status.toLowerCase()}`}>{action.status==='PLANNED'?'待人工处理':action.status}</span><div className="queue-result neutral"><b>不会自动推送</b><small>请在 Wayfair 平台核验并处理</small></div><div className="queue-actions">{canRemoveAction(action.status)?<button disabled={batchBusy} onClick={()=>removeAction(action)}>移除</button>:null}</div></article>)}{!queueLoading&&!manualQueuedActions.length?<p className="empty-state">当前没有待人工处理的 Campaign 或资格动作。</p>:null}</div>
        {batchMessage&&<div className={`inventory-message ${/(失败|未启用|必须)/.test(batchMessage)?'bad':'good'}`}>{batchMessage}</div>}
      </section>
    ...
    {tab==='ai'&&<>
    <section className="card ai-execution-boundary"><div><span>CONTROLLED API WRITE</span><h2>AI 优化 · API 一键执行</h2><p>这里只展示可通过 Advertising API 预检与写入的 Listing Bid 和启停建议。系统先 Dry-run 预检，正式写入仍需确认；Campaign、资格与平台规则请到左侧“手动优化 To-Do”处理。</p></div><b>{ready?`${ready} 项 API 建议待处理`:'当前没有可 API 执行的建议'}</b></section>
    <section className="stat-grid four ad-kpis">{[
      ...
      [loading?'-':String(ready),"可 API 执行建议",`${apiQueuedActions.length} 项已加入 API 批次`],
    ].map(([value,label,note])=><article className="stat" key={label}><strong>{value}</strong><span>{label}</span><small>{note}</small></article>)}</section>
    <section className="card queue-panel"><div className="section-head"><div><span>ADVERTISING API</span><h2>API 执行批次</h2></div><b>{queueLoading?'读取中':`${apiQueuedActions.length} 项 · 成功 ${executedActions.length} · 失败 ${failedActions.length} · 待执行 ${validatedActions.length}`}</b></div>
    ...
    <section className="card action-ledger"><div className="section-head"><div><span>API 建议</span><h2>可 API 执行的 Listing 建议</h2></div><b>{filteredListings.length} / {apiListings.length} 项</b></div>
      <div className="table-tools"><label className="search-field">搜索<input value={actionQuery} onChange={event=>setActionQuery(event.target.value)} placeholder="筛选 Listing、Campaign 或 Part"/></label><label>建议<select value={recommendationFilter} onChange={event=>setRecommendationFilter(event.target.value)}><option value="ALL">全部建议</option><option value="READY">建议调整</option></select></label><label>执行单<select value={queueFilter} onChange={event=>setQueueFilter(event.target.value)}><option value="ALL">全部状态</option><option value="unqueued">未加入</option><option value="queued">已加入</option></select></label><span>已选 {selectedRecommendations.length} 项</span><button className="primary" disabled={!selectedRecommendations.length||batchBusy} onClick={queueSelected}>批量加入 API 执行单</button></div>
