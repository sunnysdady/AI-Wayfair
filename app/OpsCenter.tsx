...
type AdAnalysis = {
  current: AdMetric; previous: AdMetric; history: ({ date: string } & AdMetric)[]; campaigns: AdCampaign[]; listings: AdListing[]; aiCampaignDiagnostics: AiCampaignDiagnosis[];
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{requested.start} 至 {requested.end}</span></div>} />
    ...
    <section className="card ai-execution-boundary"><div><span>CONTROLLED WRITE</span><h2>AI 优化 · 一键执行</h2><p>先诊断AI Campaign学习状态；仅对已通过 Gate 的 Listing Bid 与启停动作执行。学习期Campaign不会进入一键写入；系统先Dry-run预检，正式写入仍需确认。</p></div><b>{urgentAiDiagnostics?`${urgentAiDiagnostics} 项学习异常优先处理`:`${ready} 项建议待处理`}</b></section>
