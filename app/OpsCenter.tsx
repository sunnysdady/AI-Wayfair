...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  useEffect(()=>{if(tab!=='manual')return;try{const stored=JSON.parse(window.localStorage.getItem('manual-ad-todos:v1')||'[]');if(Array.isArray(stored))setManualDone(stored.filter(item=>typeof item==='string'));const resolutions=JSON.parse(window.localStorage.getItem(ZOMBIE_RESOLUTION_STORAGE_KEY)||'{}');if(resolutions&&typeof resolutions==='object'&&!Array.isArray(resolutions))setZombieResolutions(resolutions);}catch{}},[tab]);
  ...
  const pageCount=tab==='manager'?`${data?.campaigns.length||0} 个 Campaign`:tab==='listings'?`${data?.listings.length||0} 个 Listing`:tab==='manual'?`${resolvedZombieCount} / ${zombieFindings.length} 诊断已完成 · ${manualDone.length} / ${MANUAL_AD_TASKS.length} To-Do`:`${ready} 项建议 · ${validatedActions.length} 项待执行`;
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{requested.start} 至 {requested.end}</span></div>} />
    ...
    <section className="card action-ledger ai-api-workbench"><div className="section-head"><div><span>ADVERTISING API</span><h2>AI API 执行工作台</h2></div><b>{queueLoading?'读取中':`${ready} 项建议 · ${apiQueuedActions.length} 项已入批次 · 成功 ${executedActions.length} · 失败 ${failedActions.length}`}</b></div>
      <div className="ai-workbench-metrics"><span><small>所选周期花费</small><b>{loading?'-':money(data?.current.spend)}</b><em>{change(data?.current.spend,data?.previous.spend)}</em></span><span><small>归因订单</small><b>{loading?'-':String(data?.current.orders||0)}</b><em>{change(data?.current.orders,data?.previous.orders)}</em></span><span><small>WSC ROAS</small><b>{loading?'-':`${(data?.current.wscRoas||0).toFixed(2)}×`}</b><em>前周期 {(data?.previous.wscRoas||0).toFixed(2)}×</em></span><span><small>动作进度</small><b>{validatedActions.length} 待执行</b><em>{approvedActions.length} 待预检</em></span></div>
