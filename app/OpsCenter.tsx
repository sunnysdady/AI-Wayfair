...
import { queuedActionState } from "../lib/ad-action-queue.mjs";
...
function Ads() {
  ...
  return <><Hero eyebrow="WEEKLY AI AD OPTIMIZATION" title="广告周优化" text="每周自动用成熟7天数据生成SKU动作；最近7/14天用于观察，不再因为归因窗口让整张清单失去操作性" side={<div className="hero-side"><b>{loading?'同步中':error?'需检查':'本周建议已生成'}</b><span>{data?.cache?.layer==='ADVERTISING_API'?'Advertising API已入库':'D1数据库直接读取'} · 生产写入需确认</span></div>} />
    ...
    <section className="card queue-panel"><div className="section-head"><div><span>EXECUTION BATCH · PERSISTED</span><h2>本周执行批次</h2></div><b>{queueLoading?'读取中':`${queuedActions.length}项 · 待确认${plannedActions.length} · 已预检${validatedActions.length} · 已执行${executedActions.length}`}</b></div>
      <div className="queue-flow"><span className={plannedActions.length?'active':''}>1 保存批次</span><i>→</i><span className={approvedActions.length?'active':''}>2 人工确认</span><i>→</i><span className={validatedActions.length?'active':''}>3 API预检</span><i>→</i><span className={executedActions.length?'done':''}>4 执行与留痕</span></div>
      {queueError&&<div className="inline-error">{queueError}</div>}
      <div className="queue-rows">{queuedActions.map(action=>{const source=data?.listings.find(row=>row.campaignId===action.campaign_id&&row.listing===action.listing);const apiAction=['SET_LISTING_BID','SET_LISTING_ACTIVE'].includes(action.action_type);const confirmable=apiAction&&source?.action.execution==='READY_FOR_PLAN';return <article key={action.id}><div><strong>{action.listing}</strong><small>Campaign {action.campaign_id}</small></div><b>{actionPayloadText(action)}</b><span className={`queue-status ${action.status.toLowerCase()}`}>{action.status==='PLANNED'?apiAction?'待确认':'Partner Home人工任务':action.status==='APPROVED'?'已确认 · 待预检':action.status==='VALIDATED'?'预检通过 · 待执行':action.status==='EXECUTING'?'正在执行':action.status==='EXECUTED'?'已执行':'执行失败 · 可重试'}</span><div>{action.status==='PLANNED'&&apiAction?<button className="primary" disabled={!confirmable||batchBusy} onClick={()=>approveAction(action)}>确认进入 API 预检</button>:null}{action.status==='FAILED'&&apiAction?<button className="primary" disabled={batchBusy} onClick={()=>approveAction(action)}>重新进入 API 预检</button>:null}{action.status==='PLANNED'?<button disabled={batchBusy} onClick={()=>removeAction(action)}>移除</button>:null}{!confirmable&&action.status==='PLANNED'&&apiAction?<small>{source?.action.blockers.join('；')||'当前分析周期不满足执行Gate'}</small>:null}</div></article>})}{!queueLoading&&!queuedActions.length?<p className="empty-state">暂无执行项。在下方建议中点击“加入本周执行单”后，会汇总到这里并持久保留。</p>:null}</div>
