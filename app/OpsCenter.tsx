...
function Inventory({ embedded = false }: { embedded?: boolean }) {
  ...
  type InventoryPushResult={error?:string;itemCount?:number;batchCount?:number;mode?:string;feedKind?:'DIFFERENTIAL';status?:'processing'|'completed'|'failed';completedBatches?:number;failedBatches?:number;pushId?:string;batches?:{index:number;state:string;reason?:string;status?:string;completedCount?:number;processingCount?:number;errorCount?:number}[]};
  ...
  async function push(dryRun:boolean){if(!preview?.snapshotId)return;setBusy(true);setMessage('');setState(dryRun?'正在执行Dry-run':'正在提交Wayfair');try{const response=await fetch('/api/inventory/push',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({snapshotId:preview.snapshotId,dryRun,confirmation,zeroStockConfirmed:zeroConfirmed})});let body=await response.json() as InventoryPushResult;if(!response.ok)throw new Error(`${body.error||'库存推送失败'}${body.batches?.find(item=>item.state==='failed')?.reason?`：${body.batches.find(item=>item.state==='failed')?.reason}`:''}`);if(dryRun){setState('Dry-run 已通过');setMessage(`Dry-run完成：${body.itemCount}条记录，拆分${body.batchCount}个差量批次；尚未写入Wayfair。`);return;}if(body.status==='processing'&&body.pushId){setState('Wayfair 处理中');setMessage(`${body.batchCount||0}个差量批次已提交，正在核对Wayfair处理结果；回执 ${body.pushId.slice(0,8)}。`);body=await waitForPush(body.pushId,body);}if(body.status==='completed'){setState('Wayfair feed 已处理');setMessage(`Wayfair已处理差量库存feed：${body.completedBatches}/${body.batchCount}个批次、共${body.itemCount}条；回执 ${body.pushId?.slice(0,8)}。Partner Home可能有短暂展示延迟，请抽查变更SKU。`);}else if(body.status==='failed'){throw new Error(`${body.error||'Wayfair 库存批次处理失败'}${body.batches?.find(item=>item.state==='failed')?.reason?`：${body.batches.find(item=>item.state==='failed')?.reason}`:''}`);}else{setState('Wayfair 处理中');setMessage(`Wayfair 仍在处理：${body.completedBatches||0}/${body.batchCount}个批次完成。回执 ${body.pushId?.slice(0,8)}，刷新页面后可继续核对。`);}}catch(error){setState(dryRun?'Dry-run 失败':'正式推送未完成');setMessage(error instanceof Error?error.message:'库存推送失败');}finally{setBusy(false);}}
...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  useEffect(()=>{if(tab!=='manual')return;try{const stored=JSON.parse(window.localStorage.getItem('manual-ad-todos:v1')||'[]');if(Array.isArray(stored)){const completed=stored.filter((item):item is string=>typeof item==='string');const expanded=completed.flatMap(item=>{const legacyTask=MANUAL_AD_TASK_IDS.has(item)?MANUAL_AD_TASKS.find(task=>task.id===item):undefined;return legacyTask?legacyTask.parentSkus.map(sku=>manualTaskKey(sku,legacyTask.id)):[item];});setManualDone([...new Set(expanded)]);}const resolutions=JSON.parse(window.localStorage.getItem(ZOMBIE_RESOLUTION_STORAGE_KEY)||'{}');if(resolutions&&typeof resolutions==='object'&&!Array.isArray(resolutions))setZombieResolutions(resolutions);}catch{}},[tab]);
