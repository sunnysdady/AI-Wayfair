...
function Plan({ embedded = false, onOpenReview, tab, onTabChange }: { embedded?: boolean; onOpenReview: () => void; tab: PlanSection; onTabChange: (tab: PlanSection) => void }) {
  ...
  return <>{!embedded&&<Hero eyebrow="MONTHLY OPERATING PLAN" title="目标与执行" text="6月复盘 → 7月真实基线执行 → 8月下一阶段准备；目标、利润与广告共用同一套运营计划" side={<button className="hero-button" onClick={onOpenReview}>查看完整复盘证据</button>} />}
    ...
    {tab==='august'&&<><section className="august-sales-summary">{[
      ...
      [money(executionPolicy?.stageOneAdCap||salesSummary?.plannedAdBudget||0),'首阶段广告总上限',`基础 ${money(executionPolicy?.baseAdBudget||0)} + Canary最大损失 ${money(executionPolicy?.canaryLossCap||0)} · 第二阶段 ${money(executionPolicy?.stageTwoAdCap||0)}`],
    ...
    <div className="plan-workspace august-order-workspace">
      ...
      <section className="card promotion-review"><div className="section-head"><div><span>PROMOTION SUBMISSION SYNCED · 2026-07-28</span><h2>逐Part促销执行情况</h2></div><b className="promotion-lock synced">已审核并提报</b></div>
        <div className="promotion-review-summary"><article><span>活动已提报</span><strong>{promotionSummary?.submittedParts||0} Parts</strong><small>{promotionSummary?.submittedListings||0} 个Listing · 紫鸟已提交 {promotionSummary?.ziniaoSubmittedParts||0}</small></article><article><span>暂缓</span><strong>{promotionSummary?.heldParts||0} Parts</strong><small>当前仅保留低库存风险项</small></article><article><span>B2B买2额外5%</span><strong>{promotionSummary?.quantityPromotionParts||0} Parts</strong><small>项目 #{data?.nextPlan.quantityPromotion.projectId||'-'} · {promotionSummary?.quantityPromotionStatus==='PROCESSING'?'处理中':'待确认'}</small></article><article><span>促销兼容广告预算</span><strong>{money(promotionSummary?.recommendedAdBudget||0)}</strong><small>主情景广告后利润率 {percent(promotionSummary?.projectedPostAdMargin||0)}</small></article></div>
        <div className="promotion-budget-warning"><b>原销售预算 {money(promotionSummary?.originalAdBudget||0)} 已废止 → 基础 {money(promotionSummary?.recommendedAdBudget||0)}</b><span>已把活动折扣和买2件额外5%计入销售计划。主情景：{percent(promotionSummary?.projectedPromotionOrderShare||0)} 活动订单、{percent(promotionSummary?.projectedQuantityOrderShare||0)} 多件订单，按首阶段总上限测算的广告后利润率 {percent(promotionSummary?.projectedPostAdMargin||0)}；压力情景：{percent(promotionSummary?.stressPromotionOrderShare||0)} / {percent(promotionSummary?.stressQuantityOrderShare||0)}，利润率 {percent(promotionSummary?.stressPostAdMargin||0)}。低于10%立即停止扩量并把广告收紧至 {money(promotionSummary?.fallbackAdBudget||0)}。</span></div>
        <div className="promotion-review-table"><div className="promotion-review-row head"><span>Listing / Part</span><span>角色 / 证据</span><span>活动折扣 / 多件优惠</span><span>叠加后毛利</span><span>活动 / 状态</span></div>{(data?.nextPlan.promotionPlan||[]).map(item=><article className="promotion-review-row" key={`${item.listing}-${item.part}`}><span><b>{item.listing}</b><small>{item.part}</small></span><span><b className={`sales-role ${item.role.toLowerCase()}`}>{salesRoleNames[item.role]||item.role}</b><small>Partner Base {item.priceBasisCents==null?'待补':money2(item.priceBasisCents/100)} · 采购成本 {item.costCents==null?'待补':money2(item.costCents/100)} · 库存 {item.inventoryOnHand} · LIVE映射 {item.catalogLiveCount}</small></span><span><b>{item.discountPlan}</b><small>{item.memberB2cDiscount?`会员活动 B2C ${percent(item.memberB2cDiscount)} · `:''}{item.quantityOffer?`多件项目处理中 · `:''}{item.reason}</small></span><span><b className={item.marginAlert?'margin-alert':item.marginExceptionApproved?'margin-exception':''}>{item.estimatedWorstMargin==null?'待补证据':percent(item.estimatedWorstMargin)}</b><small>{item.marginAlert?'低于执行底线，禁止额外广告扩量':item.marginExceptionApproved?`已批准例外底线 ${percent(item.roleMarginFloor)} · 不追加广告`:`角色底线 ${percent(item.roleMarginFloor)}`}</small></span><span><b className={`promotion-decision ${item.action.toLowerCase()}`}>{item.action==='SUBMITTED'?'已提报':'暂缓'}</b><small>{item.activeEventIds.length?`Q3已生效 · `:''}{item.submittedEventIds.length?`${item.submittedEventIds.length}个活动已提交`:'不进入活动'}</small></span></article>)}</div>
        <footer className="promotion-review-note"><b>多件优惠：</b> #{data?.nextPlan.quantityPromotion.projectId||'-'}「{data?.nextPlan.quantityPromotion.name||'B2B Buy 2 Save 5%'}」覆盖 {promotionSummary?.quantityPromotionParts||0} 个SKU，状态为处理中；额外5%会与当时最深B2B/活动折扣叠加。当前有 {promotionSummary?.marginExceptionParts||0} 个已批准利润例外SKU、{promotionSummary?.marginAlertParts||0} 个未解决预警SKU；利润例外订单不允许额外广告扩量。</footer>
      </section></>}
  </>;
}
