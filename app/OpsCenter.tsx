...
function Ads({ tab }: { tab: AdsTab }) {
  ...
  useEffect(()=>{if(tab!=='manual')return;try{const stored=JSON.parse(window.localStorage.getItem('manual-ad-todos:v1')||'[]');if(Array.isArray(stored)){const completed=stored.filter((item):item is string=>typeof item==='string');const expanded=completed.flatMap(item=>{const legacyTask=MANUAL_AD_TASKS.find(task=>task.id===item);return legacyTask?legacyTask.parentSkus.map(sku=>manualTaskKey(sku,legacyTask.id)):[item];});setManualDone([...new Set(expanded)]);}const resolutions=JSON.parse(window.localStorage.getItem(ZOMBIE_RESOLUTION_STORAGE_KEY)||'{}');if(resolutions&&typeof resolutions==='object'&&!Array.isArray(resolutions))setZombieResolutions(resolutions);}catch{}},[tab]);
  ...
  const filteredListings=filterAdActions(apiListings,{query:actionQuery,recommendation:'ALL',queue:queueFilter},queueState) as AdListing[];
  const selectableListings=filteredListings.filter(row=>!queueActionByKey.has(`${row.campaignId}:${row.listing}:${row.action.type}`));
  ...
  return <><Hero eyebrow="" title={pageTitle} text="" side={<div className="hero-side compact-status"><b>{loading?'同步中':error?'需检查':pageCount}</b><span>{headerRange?.start||'-'} 至 {headerRange?.end||'-'}</span></div>} />
    ...
    {tab==='manual'&&<div className="manual-optimization-workspace">
      ...
      <section className="card manual-todo-card"><div className="section-head"><div><span>OPERATOR CHECKLIST</span><h2>手动优化 To-Do List · 按父体 SKU</h2></div><b>{manualDone.length} / {MANUAL_AD_TASK_COUNT} 已完成</b></div><div className="manual-todo-list">{MANUAL_AD_TASK_GROUPS.map(group=>{const completed=group.tasks.filter(task=>manualDone.includes(manualTaskKey(group.sku,task.id))).length;return <details className={`manual-sku-group${completed===group.tasks.length?' done':''}`} open key={group.sku}><summary><span><small>{group.sku==='全账户'?'ACCOUNT TASK':'PARENT SKU'}</small><strong>{group.sku}</strong></span><span><b>{completed} / {group.tasks.length} 完成</b><progress max={group.tasks.length} value={completed}/></span></summary><div className="manual-sku-actions">{group.tasks.map((task,index)=>{const taskId=manualTaskKey(group.sku,task.id);const done=manualDone.includes(taskId);return <label className={`manual-operation-item${done?' done':''}`} key={taskId}><input type="checkbox" checked={done} aria-label={`标记 ${group.sku} 的${task.title}为完成`} onChange={()=>toggleManualTask(taskId)}/><span className="manual-operation-index">{String(index+1).padStart(2,'0')}</span><div className="manual-operation-body"><header><span><em>{task.priority}</em><small>{task.group}</small></span><strong>{task.title}</strong></header><p>{task.detail}</p><dl className="manual-task-details"><div><dt>广告类型</dt><dd>{task.adType}</dd></div><div><dt>起始 Bid</dt><dd>{task.bid}</dd></div><div><dt>预算</dt><dd>{task.budget}</dd></div><div><dt>关键词</dt><dd>{task.keywords}</dd></div><div><dt>匹配</dt><dd>{task.match}</dd></div><div><dt>适用范围</dt><dd>{task.sku}</dd></div><div className="rule"><dt>执行 / 验收规则</dt><dd>{task.rule}</dd></div></dl></div></label>})}</div></details>})}</div></section>
    </div>}
    {tab==='ai'&&<>
