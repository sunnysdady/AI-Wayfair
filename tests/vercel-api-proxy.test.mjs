...
test("forwards method and query without leaking visitor credentials", async () => {
  let forwarded;
  const response = await proxySitesApi(
  ...
  assert.equal(
    forwarded.url,
    "https://wayfair-ai-ops-center.sunnysdady.chatgpt.site/api/email/daily?date=2026-07-22",
  );
...
test("rejects Outlook ingest before dispatch when its token is invalid", async () => {
  let called = false;
  const response = await proxySitesApi(
    new Request("https://ai-wayfair.vercel.app/api/email/daily", {
      method: "POST",
...
test("uses separate gateway and ingest credentials for authorized Outlook writes", async () => {
  ...
  assert.equal(
    forwarded.url,
    "https://wayfair-ai-ops-center.sunnysdady.chatgpt.site/api/email/daily",
  );
  assert.equal(forwarded.init.headers.get("oai-sites-authorization"), "Bearer test-secret-token");
